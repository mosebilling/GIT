...
#End Region
    ...
    Public Sub CheckNLoad(Optional ByVal FromTrId As Long = 0)
        ...
        Else
            InvList = _objcmnbLayer._fldDatatable("SELECT TrId FROM ItmInvCmnTb WHERE " & IIf(UsrBr = "", "", " Brid='" & UsrBr & "' AND") & " TrType = 'MIS' AND InvNo = " & Val(numVchrNo.Text) & " AND PreFix = '" & MkDbSrchStr(Trim(txtprefix.Text), True) & "'")
        End If
    ...
    Private Sub ldPostedInv()
        ...
        dtset = _objcmnbLayer._ldDataset("SELECT ItmInvCmnTb.*,jobname,[Voucher Name],Ctgry,isnull(SalesmanTb.accountno,0) Smanacc," & _
                                         "Alias,AccDescr,isnull(linkno,0)linkno,rvprefix,rvno " & _
                                                  ...
                                                  "WHERE TrId = " & loadedTrId & " AND TrType = 'MIS'" & itemquery, False)
...
Quit:
    ...
    Private Sub NextNumber()
        ...
        '    dtInv = _objcmnbLayer._fldDatatable("SELECT Prefix,InvNo FROM InvNos WHERE InvType='MIS'")
        ...
        Try
            ...
            With cmbVoucherTp
                If .Items.Count > 0 Then
                    ...
                    cmbVoucherTp.Tag = getvrsId(cmbVoucherTp.Text, 15)
                    getVrsDet(Val(cmbVoucherTp.Tag), "MIS", vrPrefix, vrVoucherNo, vrAccountNo1, vrAccountNo2, , , vtype)
                Else
                    getVrsDet(0, "MIS", vrPrefix, vrVoucherNo, vrAccountNo1, vrAccountNo2)
                End If
        ...
        With grdVoucher
            ...
            Select Case ColIndex
                Case ConstItemCode
                    ...
                    dtItms = ItmValidation(3, SrchText, True, "MIS", Val(txtSuppAlias.Tag))
    ...
    Private Sub checkItemQty(ByVal i As Integer, ByVal checkqty As Boolean, ByVal checkcost As Boolean)
        ...
        With grdVoucher
            ...
            If dtQty.Rows.Count > 0 Then
                If dtQty(0)("AsOnQty") - qty < 0 And enableNegativeQtyAlert Then
                    If diableNegativeSale Then
                        If checkqty Then
                    ...
                    End If
                End If
                If dtQty(0)("Cost") > unitprice And EnableAlertBelowcost Then
    ...
    Private Sub AddDetails(ByVal DR As DataTable)
        ...
        With grdVoucher
            ...
            If CDbl(.Item(ConstUPrice, i).Value) = 0 Or chgItm Then
                ...
                ElseIf rdodealer.Checked Then
                    .Item(ConstActualPrice, i).Value = DR(0)("secondPrice")
                ElseIf enableFuleBankInvoice Then
                    .Item(ConstActualPrice, i).Value = getLastSalesAmt(0, DR(0)("ItemId"), False, "MIS")
                ElseIf rdoretail.Checked Then
                    If Val(DR(0)("UnitPrice") & "") = 0 Then DR(0)("UnitPrice") = 0
                    If isNotCustomerAccount Then
                        .Item(ConstActualPrice, i).Value = 0
                    Else
                        .Item(ConstActualPrice, i).Value = IIf(enablefetchLastPrice, DR(0)("lastPrice"), DR(0)("UnitPrice")) ' getLastSalesAmt(Val(txtSuppAlias.Tag), DR(0)("ItemId"), True, "MIS", CDbl(DR(0)("UnitPrice")))
                    End If
                    If Val(.Item(ConstActualPrice, i).Value) = 0 Then
            ...
            Dim r As Integer
            For r = 0 To .RowCount - 1
                If Val(.Item(ConstItemID, r).Value) = DR(0)("ItemId") And r <> i Then
                    If MsgBox("Item already selected! Do you want to continue?", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo) = MsgBoxResult.No Then
                        ClearRow(i)
                    End If
                End If
            Next
        ...
        chgbyprg = False
    End Sub
    Private Sub ClearRow(ByVal i As Integer)
        Dim PMult As Double
        With grdVoucher
            chgbyprg = True
            PMult = 1
            ...
            .Item(ConstcessAc, i).Value = 0
            .Item(ConstRegcess, i).Value = Format(0, lnumFormat)
            .Item(ConstRegcessAc, i).Value = 0
            .Item(ConstAdditionalcess, i).Value = 0
            .Item(Constcess, i).Value = Format(0, lnumFormat)

            .Item(ConstDisAmt, i).Value = Format(0, lnumFormat)
            .Item(ConstLTotal, i).Value = Format(0, lnumFormat)
            .Item(ConstNUPrice, i).Value = Format(0, lnumFormat)
            .Item(ConstQty, i).Value = Format(1, lnumFormat)
            .Item(ConstImpDocId, i).Value = ""
            .Item(ConstImpLnId, i).Value = ""
            chgAmt = True
            chgAmt = True
...
nxt:
    ...
    Private Sub ClearControls()
        ...
        lbladd7.Text = ""
        lbltrdate.Text = ""
        lblmembertotal.Text = Format(0, numFormat)
        txtIgst.Text = 0
        txtIgstAmt.Text = 0
        txtCgst.Text = 0
        txtCgstAmt.Text = 0
    ...
    Private Sub saveTrans()
        ...
        If Not isModi Then
chkagain:
            If Not CheckNoExists(txtprefix.Text, Val(numVchrNo.Text), "MIS", "Inventory") Then
        ...
        If isModi = False Then
            ...
            'txtprefix.Text = SetNextVrNo(numVchrNo, Val(cmbVoucherTp.Tag), "MIS", "TrType = 'MIS' AND InvNo = ", False, True, True)
        End If
...
extc:
        ...
        If Not dtSetoffTable Is Nothing Then
            If enableMultipleDebitInInvoice And dtSetoffTable.Rows.Count > 0 Then
                ...
                For i = 0 To dtSetoffTable.Rows.Count - 1
                    ...
                    If balance > 0 Then
                        setAcctrDetValue(Val(dtSetoffTable(i)("linkno")), Val(dtSetoffTable(i)("accountno")), "ON/AC", "Collection Setoff " & Trim(dtSetoffTable(i)("EntryRef")), balance * -1, "", "", 0, 0, "", _
                               "", Val(txtSuppAlias.Tag), "", dtSetoffTable(i)("CurrencyCode"), dtSetoffTable(i)("CurrRate"), , Val(dtSetoffTable(i)("UnqNo")))

                        setAcctrDetValue(Val(dtSetoffTable(i)("linkno")), Val(dtSetoffTable(i)("accountno")), reference, "Collection Setoff " & Trim(dtSetoffTable(i)("EntryRef")), CDbl(dtSetoffTable(i)("Assign")) * -1, "", "", 0, 0, "", _
    ...
    Private Sub setInvCmnValue(ByVal InvTrid As Long, ByVal paidamt As Double)
        With _objInv
            ...
            .TypeNo = TrTypeNo ' getVouchernumber("MIS")
            .EnaJob = False
    ...
    Private Sub setInvDetValue(ByVal Invid As Long, ByVal PPerU As Single, ByVal i As Integer)
        With grdVoucher
            ...
            End If
            _objInv.SlNo = i + 1
            _objInv.TrTypeNo = TrTypeNo ' getVouchernumber("MIS")
    ...
    Private Function saveInvTr(ByVal Invid As Long) As Double
        ...
        With grdVoucher
            For i = 0 To .Rows.Count - 1
                ...
                slno = slno + 1
                dtrow("SlNo") = slno
                dtrow("TrTypeNo") = TrTypeNo ' getVouchernumber("MIS")
...
skip:
    ...
    Private Sub setAcctrCmnValue(ByVal InvTrid As Long, ByVal LinkNo As Long)
        ...
        _objTr.JVTypeNo = TrTypeNo ' getVouchernumber("MIS")
        _objTr.UserId = Trim(CurrentUser & "")
        _objTr.MchName = MACHINENAME
        _objTr.CrtDtTm = Dt
        _objTr.TypeNo = Val(cmbVoucherTp.Tag)
    ...
    Private Sub setAcctrCmnValueRV(ByVal InvTrid As Long, ByVal LinkNo As Long)
        ...
        _objTr.MchName = MACHINENAME
        _objTr.CrtDtTm = Dt
        _objTr.TypeNo = TrTypeNo ' getVouchernumber("MIS")
        '_objTr.VrDescr = Trim(txtpayee.Text)
...
ChkAgain:
        dtTable = _objcmnbLayer._fldDatatable("SELECT * FROM ItmInvCmnTb WHERE TrType = 'MIS' AND InvNo = " & Val(numVchrNo.Text) & " AND PreFix = '" & MkDbSrchStr(Trim(txtprefix.Text), True) & "'")
        If dtTable.Rows.Count > 0 Then
    ...
    Private Function chkGridvalue() As Boolean
        ...
        With grdVoucher
            ...
            'Next
            'dtSrlNo = _objcmnbLayer._fldDatatable("SELECT SerialNo,itemid FROM ItmInvTrTb Left Join ItmInvCmnTb on ItmInvTrTb.Trid=ItmInvCmnTb.trid WHERE TrType='MIS' AND ItmInvCmnTb.Trid<>" & loadedTrId)
...
ter:
    ...
    Private Sub setCustomer(Optional ByVal accid As Long = 0, Optional ByVal skipCalculate As Boolean = False, Optional ByVal GSTN As String = "", Optional ByVal skipcustomerdiscount As Boolean = True, Optional ByVal varlinkno As Long = 0)
        ...
        dt = _objcmnbLayer._fldDatatable("SELECT AccMast.accid,Alias,AccDescr,Address1,Address2,Address3,Address4," & _
                                        ...
                                        "CurrencyCode,CountryCode,GSTIN,Remarks,GrpSetOn,isnull(OpnBal,0)+isnull(bal,0) bal,custid,isnull(pricegroup,0)pricegroup " & _
        ...
        If dt.Rows.Count > 0 Then
            ...
            If skipcustomerdiscount = False Then
                ...
                End With
            End If
            If Val(dt(0)("pricegroup")) = 0 Then
                rdoretail.Checked = True
            ElseIf Val(dt(0)("pricegroup")) = 1 Then
                rdows.Checked = True
            Else
                rdodealer.Checked = True
            End If
...
els:
    ...
    Private Sub LoadReport(ByVal FileName As String, ByVal RptCaption As String, Optional ByVal ToPrint As Boolean = False, Optional ByVal pno As Integer = 0, Optional ByVal printername As String = "")
        ...
        Dim ds As DataSet
        ds = _objInv.ldInvoice("rturnInventoryDetailsForInvoicePrintMIS", pno)
        If ToPrint Then
...
u:
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    cost = getLastSalesAmt(Val(txtSuppAlias.Tag), dt(0)("ItemId"), True, "MIS", CDbl(dt(0)("UnitPrice")))
    ...
    Private Sub formLoad()
        Try
            loadInventoryFormLoadMasters(True, 15, "MIS", 3, DiscAcc, TrTypeNo)
...
nw:
        ...
        If userType Then
            btnRv.Visible = IIf(getRight(256, CurrentUser), False, True)
        End If
        setPrice()
        chkb2b.Checked = enableB2BAsDefault
    End Sub
...
getinfo:
    ...
    Private Sub btnprint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnprint.Click
        If enableDuplicateBill Then
            ...
            Dim dt As DataTable
            dt = _objcmnbLayer._fldDatatable("SELECT case when Printed='true' then 1 else 0 end p from ItmInvCmnTb where  trtype='MIS'  and  InvNo=" & Val(numPrintVchr.Text) & " and  PreFix='" & txtPPrefix.Text & "'")
            If dt.Rows.Count > 0 Then
                If dt(0)(0) = 0 Then
                    PrepareRpt("MIS", True)
                End If
            ...
            If frm.chkduplicate.Checked Then
                PrepareRpt("MIS", True, 1)
            End If
            If frm.chktriplicate.Checked Then
                PrepareRpt("MIS", True, 2)
            End If
        Else
            PrepareRpt("MIS", True)
        End If
    ...
    Public Function ImportDOs(ByVal Docids As String, Optional ByVal trasferAllItems As Boolean = False, Optional ByVal fetchcustomer As Boolean = False) As Boolean
        ...
        If trasferAllItems Then
            sRs = _objcmnbLayer._fldDatatable("SELECT * from (Select DocTranTb.*, [Item Code],isSerialNo,FraCount,isnull(itemCategory,'')itemCategory," & _
                                              ...
                                              "Discount,rndoff,SlManId FROM DocTranTb LEFT JOIN DocCmnTb ON DocCmnTb.DocId=DocTranTb.DocId " & _
            ...
            If sRs.Rows.Count > 0 Then
                For i = 0 To sRs.Rows.Count - 1
                    If i = 0 Then
                        ...
                        cmbsalesman.Text = Trim(sRs(0)("SlManId") & "")
...
createNewItem:
    ...
    Private Sub showOtherCost(ByVal CrossBr As Boolean)
        ...
        If enableGCC Then
            sRs = _objcmnbLayer._fldDatatable("SELECT AccTrDet.*, AccDescr, Alias, BranchId,isnull(vatcode,'')vatcode,isnull(vat,0)vat,isnull(paymentAC,0)paymentAC FROM AccTrDet " & _
                                          ...
                                          "WHERE JVType = 'MIS' AND JVNum = " & Val(numVchrNo.Text) & " AND PreFix = '" & MkDbSrchStr(Trim(txtprefix.Text)) & "') AND OthCost > 0  ORDER BY OthCost, DealAmt DESC")
        Else
            sRs = _objcmnbLayer._fldDatatable("SELECT AccTrDet.*, AccDescr, Alias, BranchId FROM AccTrDet " & _
                                          "LEFT JOIN AccMast ON AccMast.accid = AccTrDet.AccountNo " & _
                                          "WHERE  LinkNo IN (SELECT LinkNo FROM AccTrCmn WHERE JVType = 'MIS' AND JVNum = " & Val(numVchrNo.Text) & " AND PreFix = '" & MkDbSrchStr(Trim(txtprefix.Text)) & "') AND OthCost > 0  ORDER BY OthCost, DealAmt DESC")
        End If
    ...
    End Sub
    'Private Sub fetchPrice(ByVal skipmessage As Boolean)
    '    If skipmessage = False Then
    '        If MsgBox("Do you want change Price Type?", MsgBoxStyle.Question + MsgBoxStyle.YesNo + MsgBoxStyle.DefaultButton2) = MsgBoxResult.No Then Exit Sub
    '    End If
    ...
    Private Sub fetchPrice(ByVal skipmessage As Boolean)
        ...
        For i = 0 To grdVoucher.RowCount - 1
            With grdVoucher
                ...
                If .Item(ConstSerialNo, i).Value <> "" Then
                    batchnos = batchnos & IIf(batchnos = "", "", ",") & "'" & .Item(ConstSerialNo, i).Value & "'"
                End If
        ...
        Dim qry As String = ""
        If Not enableBatchwiseTr Then
            qry = "SELECT isnull(P1Price1,0) UnitPrice1,isnull(P1Price2,0) UnitPriceWS1,isnull(P1Price3,0)secondPrice1,isnull(PMRP1,0)MRP1," & _
              "isnull(P2Price1,0) UnitPrice2,isnull(P2Price2,0)UnitPriceWS2,isnull(P2Price3,0)secondPrice2,isnull(PMRP2,0)MRP2," & _
              "UnitPrice,UnitPriceWS,MRP,additionalcess,secondPrice," & _
              "InvItm.ItemId FROM " & _
              "InvItm " & _
              " left join InvItmPackingTb on InvItmPackingTb.itemid=invitm.itemid " & _
              "WHERE InvItm.Itemid  in (" & itemids & ")"
        Else
            qry = "SELECT UnitPrice,UnitPriceWS,secondPrice,MRP,tr.ItemId,batchno, " & _
                    "isnull(P1Price1,0) UnitPrice1,isnull(P1Price2,0) UnitPriceWS1,isnull(P1Price3,0)secondPrice1,isnull(PMRP1,0)MRP1," & _
                    "isnull(P2Price1,0) UnitPrice2,isnull(P2Price2,0)UnitPriceWS2,isnull(P2Price3,0)secondPrice2,isnull(PMRP2,0)MRP2 " & _
                    "FROM (select case when isnull(SerialNo,'')='' then  trtype+prefix+convert(varchar,invno)  ELSE SerialNo end batchno," & _
                    "ItmInvTrTb.ItemId," & _
                    "case when isnull(ItmInvTrTb.MRP,0)=0 then InvItm.MRP else ItmInvTrTb.MRP end MRP," & _
                    "case when isnull(SP1,0)=0 then UnitPrice else SP1 end UnitPrice,case when isnull(SP2,0)=0 then  secondPrice else SP2 end secondPrice," & _
                    "case when isnull(SP3,0)=0 then UnitPriceWS  else SP3 end UnitPriceWS FROM ItmInvTrTb " & _
                    "left join VoucherTypeNoTb on ItmInvTrTb.TrTypeNo=VoucherTypeNoTb.vrno " & _
                    "right join invitm on ItmInvTrTb.itemid=InvItm.ItemId " & _
                    "left join ItmInvCmnTb on ItmInvCmnTb.trid=ItmInvTrTb.TrId  where invstatus=0 AND invtype='IN')TR  " & _
                    "left join InvItmPackingTb on InvItmPackingTb.itemid=tr.itemid " & _
                    "where tr.Itemid  in (" & itemids & ") and batchno in (" & batchnos & ")"
        End If

        'With grdVoucher
        'For i = 0 To .RowCount - 1
        '    If qry <> "" Then qry = qry & " union all "
        '    If .Item(ConstSerialNo, i).Value <> "" And enableBatchwiseTr Then

        '    Else
        '        If .Item(ConstB, .CurrentCell.RowIndex).Value = "P1" Then
        '            qry = qry & "SELECT isnull(P1Price1,0) UnitPrice,isnull(P1Price2,0) UnitPriceWS,isnull(P1Price3,0)secondPrice,isnull(PMRP1,0)MRP,InvItm.ItemId FROM " & _
        '                                     "InvItm " & _
        '                                     " left join InvItmPackingTb on InvItmPackingTb.itemid=invitm.itemid " & _
        '                                     "WHERE InvItm.Itemid =" & Val(.Item(ConstItemID, i).Value)

        '            'dt = _objcmnbLayer._fldDatatable("SELECT P1Unit U,P1Fra F,isnull(P1Price1,0) UnitPrice,isnull(P1Price2,0) UnitPriceWS,isnull(P1Price3,0)secondPrice,isnull(PMRP1,0)MRP,InvItm.ItemId FROM " & _
        '            '                                 "InvItm " & _
        '            '                                 " left join InvItmPackingTb on InvItmPackingTb.itemid=invitm.itemid " & _
        '            '                                 "WHERE InvItm.Itemid in (" & itemids & ")")
        '        ElseIf .Item(ConstB, .CurrentCell.RowIndex).Value = "P2" Then
        '            qry = qry & "SELECT isnull(P2Price1,0) UnitPrice,isnull(P2Price2,0)UnitPriceWS,isnull(P2Price3,0)secondPrice,isnull(PMRP2,0)MRP,InvItm.ItemId FROM " & _
        '                                             "InvItm " & _
        '                                             " left join InvItmPackingTb on InvItmPackingTb.itemid=invitm.itemid " & _
        '                                             "WHERE InvItm.Itemid =" & Val(.Item(ConstItemID, i).Value)
        '            'dt = _objcmnbLayer._fldDatatable("SELECT P2Unit U,P2Fra F,isnull(P2Price1,0) UnitPrice,isnull(P2Price2,0)UnitPriceWS,isnull(P2Price3,0)secondPrice,isnull(PMRP2,0)MRP,InvItm.ItemId FROM " & _
        '            '                                 "InvItm " & _
        '            '                                 " left join InvItmPackingTb on InvItmPackingTb.itemid=invitm.itemid " & _
        '            '                                 "WHERE InvItm.Itemid in (" & itemids & ")")
        '        ElseIf .Item(ConstB, .CurrentCell.RowIndex).Value = "B" Then
        '            qry = qry & "SELECT UnitPrice,UnitPriceWS,MRP,additionalcess,secondPrice,ItemId FROM InvItm WHERE Itemid =" & Val(.Item(ConstItemID, i).Value)
        '            'dt = _objcmnbLayer._fldDatatable("SELECT Unit U,1 F,UnitPrice,UnitPriceWS,MRP,additionalcess,secondPrice,ItemId FROM InvItm WHERE Itemid in (" & itemids & ")")
        '        End If
        '    End If
        'Next
        'If .Item(ConstB, .CurrentCell.RowIndex).Value = "P1" Then
        ...
        For i = 0 To grdVoucher.RowCount - 1
            With grdVoucher
                itemid = Val(.Item(ConstItemID, i).Value)
                If enableBatchwiseTr Then
                    _qurey = From data In dt.AsEnumerable() Where data("itemid") = itemid And data("batchno") = .Item(ConstSerialNo, i).Value Select data
                Else
                    _qurey = From data In dt.AsEnumerable() Where data("itemid") = itemid Select data
                End If
                If _qurey.Count > 0 Then
                ...
                If bDatatable.Rows.Count > 0 Then
                    If rdoretail.Checked Then
                        If .Item(ConstB, .CurrentCell.RowIndex).Value = "P1" Then
                            .Item(ConstActualPrice, i).Value = bDatatable(0)("UnitPrice1")
                            .Item(ConstMRP, i).Value = bDatatable(0)("MRP1")
                        ElseIf .Item(ConstB, .CurrentCell.RowIndex).Value = "P2" Then
                            .Item(ConstActualPrice, i).Value = bDatatable(0)("UnitPrice2")
                            .Item(ConstMRP, i).Value = bDatatable(0)("MRP2")
                        Else
                            .Item(ConstActualPrice, i).Value = bDatatable(0)("UnitPrice")
                            .Item(ConstMRP, i).Value = bDatatable(0)("MRP")
                        End If
                    ElseIf rdodealer.Checked Then
                        If .Item(ConstB, .CurrentCell.RowIndex).Value = "P1" Then
                            .Item(ConstActualPrice, i).Value = bDatatable(0)("secondPrice1")
                        ElseIf .Item(ConstB, .CurrentCell.RowIndex).Value = "P2" Then
                            .Item(ConstActualPrice, i).Value = bDatatable(0)("secondPrice2")
                        Else
                            .Item(ConstActualPrice, i).Value = bDatatable(0)("secondPrice")
                        End If
                    ...
                    Else
                        If .Item(ConstB, .CurrentCell.RowIndex).Value = "P1" Then
                            .Item(ConstActualPrice, i).Value = bDatatable(0)("UnitPriceWS1")
                        ElseIf .Item(ConstB, .CurrentCell.RowIndex).Value = "P2" Then
                            .Item(ConstActualPrice, i).Value = bDatatable(0)("UnitPriceWS2")
                        Else
                            .Item(ConstActualPrice, i).Value = bDatatable(0)("UnitPriceWS")
                        End If
                    End If
    ...
    End Sub
    Public Sub returnFromJobCarWash(ByVal jobcode As String, ByVal jobid As Integer, ByVal plateno As String, ByVal cartype As String, ByVal customerid As Integer)
        chgbyprg = True
        txtJob.Text = jobcode
        txtJob.Tag = jobid
        'txtjobname.Text = jobname
        Dim dt As DataTable
        dt = _objcmnbLayer._fldDatatable("Select Alias,AccDescr from AccMast where AccId=" & customerid)
        txtSuppAlias.Text = dt(0)("Alias")
        txtSuppName.Text = dt(0)("AccDescr")
        txtSuppAlias.Tag = customerid
        txtCashCustomer.Text = dt(0)("AccDescr")
        txtCashCustomer.Tag = customerid
        chgbyprg = True
        Dim PreFixTb As DataTable = _objcmnbLayer._fldDatatable("SELECT  top 1 * FROM PreFixTb WHERE vrTypeNo = 4 and ctgry=3", False)
        If PreFixTb.Rows.Count > 0 Then
            cmbVoucherTp.Text = PreFixTb(0)("Voucher Name")
        End If
createNewItem:
        Dim dR As DataTable = ItmValidation(1, plateno, True, "MIS", Val(txtSuppAlias.Tag))
        If dR.Rows.Count > 0 Then
            grdVoucher.Rows.Add(1)
            grdVoucher.CurrentCell = grdVoucher.Item(0, (grdVoucher.RowCount - 1))
            AddDetails(dR)
    ...
    Private Sub rdoretail_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles rdoretail.Click, rdodealer.Click, rdows.Click
        fetchPrice(False)
    End Sub

    Private Sub ldtimer_Tick(ByVal sender As Object, ByVal e As System.EventArgs) Handles ldtimer.Tick
        If chgbyprg Then Exit Sub
        ldtimer.Enabled = False
        chgbyprg = True
        ShowPanel()
    ...
    Public Sub MenuStripClick(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        End Select
    End Sub
    Public Sub returnFromJob(ByVal jbid As Long, ByVal docid As Long)
        Dim dt As DataTable
        dt = _objcmnbLayer._fldDatatable("select jobcode,jobname,Alias,AccDescr,custcode from jobtb lEFT JOIN AccMast on JobTb.custcode=Accmast.accid " & _
                                        "LEFT JOIN AccMastAddr ON Accmast.accid =AccMastAddr.AccountNo  where jobid=" & jbid)
        If dt.Rows.Count > 0 Then
            chgbyprg = True
            ...
            txtSuppName.Text = dt(0)("AccDescr")
            txtCashCustomer.Text = dt(0)("AccDescr")
            txtSuppName.Tag = dt(0)("custcode")
            setCustomer(Val(txtSuppName.Tag))
            chgbyprg = False
            If docid > 0 Then
                ImportDOs(docid.ToString, True)
            End If
        End If
    ...
    Private Sub setGridHeadMember()
        ...
        With grdmember
            ...
            .Columns(mValue).ReadOnly = True

            .Columns(mMemberid).Visible = False
        End With
        chgbyprg = False
    End Sub
    Private Sub AddMemberRow()
        Dim i As Integer
        With grdmember
            If .RowCount = 5 Then
                MsgBox("Only five members are allowed", MsgBoxStyle.Exclamation)
                Exit Sub
            End If
            If CDbl(lblNetAmt.Text) = 0 Then
                MsgBox("Invalid Net Amount", MsgBoxStyle.Exclamation)
                Exit Sub
            End If
            'If .RowCount > 0 Then
            '    If .Item(mMembername, .RowCount - 1).Value = "" Then
            '        .CurrentCell = .Item(mMembername, .RowCount - 1)
            '        Exit Sub
            '    End If
            'End If
            ...
            .Item(mMembername, i).Value = ""
            .Item(mMemberid, i).Value = ""
            .Item(mValue, i).Value = Format(CDbl(lblNetAmt.Text) / 5, lnumFormat)
            .CurrentCell = .Item(mMembername, i)
            chgbyprg = True
            .BeginEdit(True)
            chgbyprg = False
        End With
    ...
    Private Sub grdmember_CellEndEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdmember.CellEndEdit
        If chgbyprg Then Exit Sub
        Dim col As Integer = e.ColumnIndex
        Dim ndec1 As Integer
        chgbyprg = True
...
nxt:
    ...
    Private Sub btnaddmemner_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnaddmemner.Click
        AddMemberRow()
    End Sub
    Private Sub calculateAssigned()
        ...
        With grdmember
            For i = 0 To .Rows.Count - 1
                If Val(.Item(mValue, i).Value) = 0 Then .Item(mValue, i).Value = 0
                ttl = ttl + CDbl(.Item(mValue, i).Value)
            Next
        End With
        lblmembertotal.Text = Format(ttl, numFormat)
    End Sub
    Private Sub saveMember(ByVal trid As Long)
        Dim i As Integer
        Dim qry As String = ""
        With grdmember
            ...
            For i = 0 To .RowCount - 1
                If .Item(mMembername, i).Value <> "" And Val(.Item(mValue, i).Value) > 0 Then
                    If Val(.Item(mMemberid, i).Value) > 0 Then
                        qry = qry & " update MicroFinanceMemberTb set membername='" & .Item(mMembername, i).Value & _
                        "',amount=" & CDbl(.Item(mValue, i).Value) & ",setremove=0 where memberid=" & Val(.Item(mMemberid, i).Value)
                    Else
                        qry = qry & " Insert into MicroFinanceMemberTb (membername,amount,trid) values('" & _
                        .Item(mMembername, i).Value & "'," & CDbl(.Item(mValue, i).Value) & "," & trid & ")"
                    End If
                End If
            Next
            qry = qry & " delete from MicroFinanceMemberTb where trid=" & trid & " and isnull(setremove,0)=1"
            _objcmnbLayer._saveDatawithOutParm(qry)
        End With
    End Sub
    Private Sub loadMember(ByVal trid As Long)
        Dim dt As DataTable
        dt = _objcmnbLayer._fldDatatable("Select * from MicroFinanceMemberTb where trid=" & trid)
        Dim i As Integer
        Dim r As Integer
        With grdmember
            .Rows.Clear()
            For i = 0 To dt.Rows.Count - 1
                ...
                .Item(mValue, i).Value = Format(dt(i)("amount"), numFormat)

...
#End Region
    ...
    Private Sub btnupdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnupdate.Click
        ...
        If txtcode.Text = "" Then
            MsgBox("Invalid Doctor Code!", MsgBoxStyle.Exclamation)
            txtcode.Focus()
            Exit Sub
        End If
        If txtname.Text = "" Then
            MsgBox("Invalid Doctor Name!", MsgBoxStyle.Exclamation)
            txtname.Focus()
            Exit Sub
        End If
        Dim dt As DataTable
        ...
        If dt.Rows.Count > 0 Then
            If dt(0)("salesmanid") <> Val(txtcode.Tag) Then
                MsgBox("Doctor Code already exist", MsgBoxStyle.Exclamation)
                txtcode.Focus()
                Exit Sub
            End If
        End If
        ...
        If dt.Rows.Count > 0 Then
            If dt(0)("salesmanid") <> Val(txtcode.Tag) Then
                MsgBox("Doctor Name already exist", MsgBoxStyle.Exclamation)
                txtname.Focus()
                Exit Sub
            End If
        End If
        ...
        If Val(txtcode.Tag) > 0 Then
            _objcmnbLayer._saveDatawithOutParm("Update SalesmanTb set SManCode ='" & txtcode.Text & "', SManName='" & txtname.Text & _
                                               ...
                                               ",empcode ='" & txtempcode.Text & "',phone='" & txtphone.Text & "' where salesmanid=" & Val(txtcode.Tag))
        Else
            _objcmnbLayer._saveDatawithOutParm("Insert into SalesmanTb (SManCode,SManName,accountno,comP,DisP,targetAmt,empcode,isdoctor,phone) values('" & _
            txtcode.Text & "','" & txtname.Text & "'," & Val(txtac.Tag) & "," & CDbl(numcom.Text) & "," & CDbl(numdis.Text) & "," & CDbl(txttargetAmt.Text) & _
            ",'" & txtempcode.Text & "',1,'" & txtphone.Text & "')")
        End If
    ...
    Private Sub loadSalesman()
        Try
            ...
            ListTb = _objcmnbLayer._fldDatatable("SELECT SManCode,SManName,ISNULL(AccDescr,'') AccDescr,SalesmanTb.comP,DisP,SalesmanTb.accountno," & _
                                                 "salesmanid,isnull(targetAmt,0)targetAmt,empcode,phone FROM SalesmanTb " & _
                                                 "LEFT JOIN AccMast ON SalesmanTb.accountno=AccMast.Accid where isnull(isdoctor,0)=1")
            With lstContent
                .Items.Clear()
                If ListTb.Rows.Count > 0 Then
                    For i = 0 To ListTb.Rows.Count - 1
                        ...
                        If .Items.Item(i).SubItems.Count > 7 Then
                            .Items.Item(i).SubItems(7).Text = .Items.Add(ListTb(i)("salesmanid"))
                        Else
                            .Items.Item(i).SubItems.Insert(7, New System.Windows.Forms.ListViewItem.ListViewSubItem(Nothing, Trim(ListTb(i)("salesmanid") & "")))
                        End If
                        If Trim(ListTb(i)("phone") & "") = "" Then ListTb(i)("phone") = ""
                        If .Items.Item(i).SubItems.Count > 8 Then
                            .Items.Item(i).SubItems(8).Text = .Items.Add(ListTb(i)("phone"))
                        Else
                            .Items.Item(i).SubItems.Insert(8, New System.Windows.Forms.ListViewItem.ListViewSubItem(Nothing, ListTb(i)("phone")))
                        End If
                    Next
                End If
            End With
    ...
    Private Sub lstContent_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles lstContent.DoubleClick
        ...
        txtphone.Text = lstContent.SelectedItems(0).SubItems(8).Text
        txtcode.Tag = Val(lstContent.SelectedItems(0).SubItems(7).Text)
        loadSlab()
        chgbyprg = False
    End Sub
    ...
    Private Sub SalesMan_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not userType Then
            ...
            btndelete.Tag = 1
        Else
            btnupdate.Tag = IIf(getRight(230, CurrentUser), 1, 0)
            btndelete.Tag = IIf(getRight(231, CurrentUser), 1, 0)
        End If
    End Sub

...
Public Class SwimmingPoolMemberShipFrm
...
#Region "Form Objects"
    ...
    Private WithEvents fshowattendance As ShowAttendanceFrm
...
#End Region
    ...
    Private Sub setGridHead()
        ...
        With grdpackage
            ...
            .Columns(ConstStartDate).Width = 100
            ...
            .Columns(ConstEndDate).Width = 100
            ...
            .Columns(ConstPrice).ReadOnly = True

            .Columns(ConstIsclosed).HeaderText = "Is Closed?"
            .Columns(ConstIsclosed).Visible = False
            ...
            .Columns(constDays).HeaderText = "Attennded"
            .Columns(constDays).Width = 70
            .Columns(constDays).SortMode = DataGridViewColumnSortMode.NotSortable
            .Columns(constDays).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
    ...
    '    Private Sub picCloseProd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    '        plsrch.Visible = False
    '    End Sub

    '    Private Sub grdSrch_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs)
    '        activecontrolname = "grdpackage"
    '        doSelect(2)
    '        chgbyprg = True
    ...
    Private Sub clearControl()
        ...
        lblinvoicetotal.Text = Format(0, numFormat)
    ...
    Private Sub saveMembership()
        ...
        With _objJob
            ...
            .imagename = FileName
            .moccupation = txtoccupation.Text
            .mmedicalcondition = txtmedicalcondition.Text
            .emergencynumber = txtemeregencynumber.Text
        ...
        If picMember.Tag <> "" Then
            ...
            If FileExists(DPath & FileName) Then
                ...
                If Not bm Is Nothing Then
                    ...
                    bm = Nothing
                End If
                bm = New Bitmap(picMember.Image)
                picMember.Image = Nothing
                picMember.BackgroundImage = Nothing
                'bm.Dispose()
                System.IO.File.Delete(DPath & FileName)
            End If
            If picMember.Tag = "fromcam" Then
                FileName = DPath & FileName
                bm.Save(FileName, Imaging.ImageFormat.Jpeg)
                'picMember.Image.Save(FileName, Imaging.ImageFormat.Jpeg)
            Else
        ...
        Try
            ...
            With _objJob
                ...
                ElseIf rdoduelist.Checked Then
                    .Tp = 5
                End If
                Dim enabledate As Integer = 0
                If chkenabledate.Checked Then
                    If chkjobdate.Checked Then
                        enabledate = 2
                    Else
                        enabledate = 1
                    End If
                End If
                _vtable = .returnPoolMembershipdetails(enabledate).Tables(0)
            End With
    ...
    Private Sub SetGrid()
        With grdList
            ...
            Else
                ...
                .Columns("Expired").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter

                .Columns("courseduration").HeaderText = "Days"
                .Columns.Item("courseduration").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                .Columns.Item("courseduration").Visible = False
                .Columns("cntattendance").HeaderText = "Attended"
                .Columns.Item("cntattendance").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                .Columns("Remaining").HeaderText = "Remaining"
                .Columns.Item("Remaining").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                .Columns("units").HeaderText = "Classes"
                .Columns.Item("units").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                '.Columns("closed").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
                resizeGridColumn(grdList, 3)
                Dim i As Integer
                For i = 0 To .RowCount - 1
                    Dim edate As Date
                    Dim expired As String = .Item("Expired", i).Value
                    If Not IsDBNull(.Item("enddate", i).Value) Then
                        edate = .Item("enddate", i).Value
                    Else
                        edate = DateValue(Date.Now)
                    End If
                    If expired = "YES" Then
                        .Rows(i).DefaultCellStyle.BackColor = Color.Red
                    ElseIf DateValue(edate) <= DateAdd(DateInterval.Day, 5, Date.Now) And DateValue(edate) > DateValue(Date.Now) Then
    ...
    Private Sub rdoactive_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles rdoactive.Click, rdoall.Click, rdoExpirylist.Click, rdoexpired.Click, rdoduelist.Click
        If rdoExpirylist.Checked Or rdoall.Checked Or rdoexpired.Checked Then
            pldate.Enabled = True
            chkenabledate.Checked = True
            If rdoall.Checked Or rdoexpired.Checked Then
                chkenabledate.Enabled = True
            Else
                chkenabledate.Enabled = False
            End If
            chkjobdate.Enabled = rdoall.Checked
            chkjobdate.Checked = rdoall.Checked
        Else
            pldate.Enabled = False
            chkenabledate.Checked = False
            chkenabledate.Enabled = True
            chkjobdate.Enabled = False
        End If
    ...
    Private Sub verify()
        ...
        If Val(txtcustomer.Tag) = 0 Then
            ...
            Exit Sub
        End If
        If txtemeregencynumber.Text = "" Then
            MsgBox("Invalid Emergency Number", MsgBoxStyle.Exclamation)
            txtemeregencynumber.Focus()
            Exit Sub
        End If
        If txtcontactname.Text = "" Then
            MsgBox("Invalid Contact Name", MsgBoxStyle.Exclamation)
    ...
    Private Sub loadMembershipForEdit()
        ...
        Try
            ...
            With _objJob
                ...
                .DateTo = DateValue(cldrEnddate.Value)
                .Tp = 3
                ds = .returnPoolMembershipdetails(0)
            End With
            dt = ds.Tables(0)
            If dt.Rows.Count > 0 Then
                ...
                txtoccupation.Text = Trim(dt(0)("moccupation") & "")
                txtmedicalcondition.Text = Trim(dt(0)("mmedicalcondition") & "")

                txtemeregencynumber.Text = Trim(dt(0)("emergencynumber") & "")
                txtcontactname.Text = Trim(dt(0)("contactname") & "")
        ...
        btncamera.Enabled = True
    End Sub
    Private Sub SetGridHeadHistory()
        With grdhistory
            SetGridProperty(grdhistory)
            .Columns("item code").HeaderText = "Package"
            .Columns("item code").Width = 75
            .Columns("item code").SortMode = DataGridViewColumnSortMode.NotSortable
            ...
            .Columns("startdate").Width = 100
            .Columns("startdate").SortMode = DataGridViewColumnSortMode.NotSortable
            .Columns("startdate").HeaderText = "Start Date"
            .Columns("startdate").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter

            .Columns("enddate").Width = 100
            .Columns("enddate").SortMode = DataGridViewColumnSortMode.NotSortable
            .Columns("enddate").HeaderText = "End Date"
            .Columns("enddate").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
            ...
            .Columns("price").DefaultCellStyle.Format = "N" & NoOfDecimal
            .Columns("price").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
            .Columns("price").DefaultCellStyle.BackColor = Color.GreenYellow

            .Columns("invno").HeaderText = "Inovice No"
            .Columns("invno").ReadOnly = True

            .Columns("cntattendance").HeaderText = "Attennded"
            .Columns("cntattendance").Width = 70
            .Columns("cntattendance").SortMode = DataGridViewColumnSortMode.NotSortable
            .Columns("cntattendance").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
        End With
    End Sub
    Private Sub loadPackage(ByVal dt As DataTable)
        ...
        With grdpackage
            ...
            For i = 0 To dt.Rows.Count - 1
                ...
                .Item(constDays, i).Value = Val(dt(i)("cntattendance") & "")
                '.Item(ConstItemid, i).Value = dt(i)("itemid")
                .Item(Constrenewid, i).Value = dt(i)("invid")
    ...
    Public Sub fillGrid()
        ...
        strSql = strSql & vbCrLf & "UNION ALL SELECT JVType [Type], prefix+ case when prefix=''  then '' else '-' end + convert(varchar(12), JVNum) [RV No],JVDate [RV Date],DealAmt*-1 Amount,Reference,Accdescr [Paid By],dbtr.ChqNo [Chq No],dbtr.ChqDate [Chq Date],dbtr.BankCode [Bank Code],JVTypeNo,AccTrCmn.Linkno From AccTrCmn " & _
                    "LEFT JOIN AccTrDet ON AccTrDet.Linkno=AccTrCmn.Linkno " & _
                    "LEFT JOIN (SELECT Linkno,Accdescr,ChqNo,ChqDate,BankCode FROM AccTrDet " & _
                    "LEFT JOIN AccMast On AccTrDet.accountno=AccMast.Accid where DealAmt>0 and trinf=10) dbtr ON AccTrDet.Linkno=dbtr.Linkno " & _
                    "where JobCode ='" & txtcode.Text & "' and JobCode<>'' and JVType='IS' and DealAmt<0 and trinf=10 and accountno=" & Val(txtcustomer.Tag)
        ...
        With grdreceipt
            For i = 0 To .RowCount - 1
                If .Item("type", i).Value = "RV" Or .Item("type", i).Value = "IS" Then
        ...
        lblbalance.Text = Format(num2 - totalRv, numFormat)
    ...
    Private Sub grdreceipt_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles grdreceipt.DoubleClick
        ...
        If grdreceipt.Item("type", grdreceipt.CurrentRow.Index).Value = "RV" Then
            fMainForm.LoadRV(Val(grdreceipt.Item("linkno", grdreceipt.CurrentRow.Index).Value), txtcustomer.Text)
            btnaddreceipt.Tag = 1
        Else
            MsgBox("Transaction Type is " & grdreceipt.Item("type", grdreceipt.CurrentRow.Index).Value & vbCrLf & " You cannot open from here", MsgBoxStyle.Exclamation)
        End If
    ...
    Private Sub btncreateinvoice_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btncreateinvoice.Click
        ...
        fInvoice.returnPoolMemberShip(Val(txtcode.Tag))
    End Sub
    ...
    Private Sub btnclosejob_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
            ...
            If _dtRptTable Is Nothing Then
                With _objJob
                    ...
                    If chkenabledate.Checked Then
                        ...
                        End If
                    End If
                    ds = .returnPoolMembershipdetails(enabledate)
                End With
            Else
    ...
    Private Sub SwimmingPoolMemberShipFrm_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        txtcustomer.Focus()
    End Sub

    Private Sub SwimmingPoolMemberShipFrm_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        If Not fDoc Is Nothing Then fDoc.Close() : fDoc = Nothing
        If Not fSelect Is Nothing Then fSelect.Close() : fSelect = Nothing
        If Not fCrtAcc Is Nothing Then fCrtAcc.Close() : fCrtAcc = Nothing
        If Not fProductEnquiry Is Nothing Then fProductEnquiry.Close() : fProductEnquiry = Nothing
        If Not frm Is Nothing Then frm.Close() : frm = Nothing
        If Not fRptFormat Is Nothing Then fRptFormat.Close() : fRptFormat = Nothing
        If Not fMList Is Nothing Then fMList.Close() : fMList = Nothing
        If Not fInvoice Is Nothing Then fInvoice.Close() : fInvoice = Nothing
        _objJob = Nothing
    ...
    Private Sub SwimmingPoolMemberShipFrm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        AddNew()
        setGridHead()
        chkenabledate.Checked = True
        cldrEnddate.Value = DateAdd(DateInterval.Day, Val(txtafter.Text), DateValue(cldrStartDate.Value))
        ldJobdetails()
        If userType Then
    ...
    Private Sub txtoccupation_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtoccupation.KeyDown
        If e.KeyCode = Keys.Return Then
            txtmedicalcondition.Focus()
        End If
    End Sub
    ...
    Private Sub chkenabledate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles chkenabledate.Click
        If chkenabledate.Checked = True Then
            pldate.Enabled = True
            If rdoexpired.Checked = False Then
                chkjobdate.Enabled = True
            End If
    ...
    Private Sub rdoExpirylist_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rdoExpirylist.CheckedChanged
    ...
    Private Sub txtafter_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtafter.Validated
        If Val(txtafter.Text) > 0 Then
            ...
            chkenabledate.Enabled = False
        Else
            If pldate.Enabled Then
                cldrStartDate.Enabled = True
            End If
            chkenabledate.Checked = False
            chkenabledate.Enabled = True
        End If
    ...
    Private Sub btnattendance_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnattendance.Click
        If grdpackage.RowCount = 0 Then Exit Sub
        If grdpackage.CurrentRow Is Nothing Then Exit Sub
        If fshowattendance Is Nothing Then
            fshowattendance = New ShowAttendanceFrm
            fshowattendance.Left = (Me.Width / 2) - (fshowattendance.Width / 2)

            'Else
            '    fshowattendance.Left = (Me.Width / 2) - (fshowattendance.Width / 2)
            '    fshowattendance.Focus()
        End If
        With grdpackage
            fshowattendance.invtrid = .Item(Constrenewid, .CurrentRow.Index).Value
            fshowattendance.cldrdate.Value = .Item(ConstStartDate, .CurrentRow.Index).Value
            fshowattendance.cldrEnddate.Value = .Item(ConstEndDate, .CurrentRow.Index).Value
            fshowattendance.lbldetails.Text = .Item(ConstDescr, .CurrentRow.Index).Value
            fshowattendance.loadAttendance()
            'fshowattendance.Top = Me.Top + 100
            resizeGridColumn(fshowattendance.grdattendancelist, 7)
            fshowattendance.ShowDialog()
        End With
    End Sub

    Private Sub fshowattendance_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles fshowattendance.FormClosed
        fshowattendance = Nothing
    End Sub

    Private Sub btncamera_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btncamera.Click
        Dim fc As New CameraFrm
        fc.ShowDialog()
        picMember.Image = Nothing
        picMember.BackgroundImage = Nothing
        On Error Resume Next
        If fc.btncapture.Tag = 1 Then
            picMember.Image = fc.picimage.Image
            picMember.Tag = "fromcam"
    ...
    Private Sub PrepareRpt(ByVal RptType As String, Optional ByVal Forprint As Boolean = False, Optional ByVal isF As Boolean = False)
        ...
        If Trim(RptName) <> "" Then
            LoadFinReport(RptName, RptCaption, Forprint)
        End If
    End Sub
    Private Sub LoadFinReport(ByVal FileName As String, ByVal RptCaption As String, Optional ByVal ToPrint As Boolean = False)
        If Not frm Is Nothing Then
            frm.Close()
            frm = New ReprtviewNEWfrm
        Else
        ...
        Dim dt1 As Date
        Dim dt2 As Date
        Dim accid As Long = Val(txtcustomer.Tag)
    ...
    Private Sub grdreceipt_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdreceipt.CellContentClick
    ...
    Private Sub txtemeregencynumber_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtemeregencynumber.KeyDown
        If e.KeyCode = Keys.Return Then
            SendKeys.Send("{TAB}")
        End If
    End Sub

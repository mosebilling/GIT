...
#End Region
    ...
    Private Sub ldPostedInv()
        ...
        cmbtunur.SelectedIndex = Val(ItmInvCmnTb(0)("turnurid") & "")
...
getcust:
    ...
    Private Sub SetGridHead()
        ...
        With grdVoucher
            ...
            .Columns(ConstQty).HeaderText = "Duration"
            .Columns(ConstQty).Width = 80
            '.Columns(ConstManufacturingdate).Visible = False
            
            .Columns(ConstTaxP).HeaderText = "VAT%"
            .Columns(ConstTaxAmt).HeaderText = "VAT Amt"
            Dim i As Integer
    ...
    Private Sub calcualteLineTotal(ByVal RowIndex As Integer)
        ...
        With grdVoucher
            ...
            ElseIf enableGCC Or ShowTaxOnInventory Then
                ...
                gstamt = CDbl(.Item(ConstTaxAmt, i).Value)
            End If
            If enablecess Then
...
ctn:
    ...
    Private Sub AddDetails(ByVal DR As DataTable)
        ...
        With grdVoucher
            ...
            .Item(ConstQty, i).Value = Format(Val(DR(0)("courseduration") & ""), "0")
            .Item(ConstImpDocId, i).Value = ""
            .Item(ConstImpLnId, i).Value = ""
            chgAmt = True
            chgAmt = True
...
nxt:
    ...
    Private Sub Verify()
        ...
        If enableBranch And UsrBr = "" Then
            ...
            Exit Sub
        End If
        If cmbtunur.Text = "" Then
            MsgBox("Invalid Payment Mode")
        End If
...
chkagain:
        ...
        TrId = Val(_objInv._saveCmn())
        _objcmnbLayer._saveDatawithOutParm("update ItmInvCmnTb set turnurid=" & cmbtunur.SelectedIndex & " where trid=" & TrId)
    ...
    Public Sub returnLodgeServices(ByVal roomid As Long, ByVal isserviceAmt As Boolean, Optional ByVal donotCalculate As Boolean = False)
        ...
        With grdVoucher
            If isserviceAmt Then
                ...
                End If
createNewItem:
                Dim dR As DataTable = getItmDtls(3, "COURSE", True)
                If dR.Rows.Count > 0 Then
                    grdVoucher.Rows.Add(1)
                    grdVoucher.CurrentCell = grdVoucher.Item(0, (grdVoucher.RowCount - 1))
                    AddDetails(dR)
                    grdVoucher.Item(ConstQty, (grdVoucher.RowCount - 1)).Value = Format(Val(dR(0)("courseduration") & ""), numFormat)
    ...
    Private Sub Save_instalment(ByVal trid As Long)
        Dim NoOfMnth As Integer
        Dim NoOfInstallment As Integer
        Dim emidate As Date
        Dim netamt As Double
        Dim i As Integer
        Dim qty As Integer
        For i = 0 To grdVoucher.RowCount - 1
            qty = qty + Val(grdVoucher.Item(ConstQty, i).Value)
        Next
        If qty = 0 Then Exit Sub
        Select Case cmbtunur.SelectedIndex
            Case 0
                NoOfInstallment = qty
                NoOfMnth = 1
            Case 1
                NoOfInstallment = qty / 3
                NoOfMnth = 3
            Case 2
                NoOfInstallment = qty / 6
                NoOfMnth = 6
            Case 3
                NoOfInstallment = qty / 12
                NoOfMnth = 12
        End Select
        Dim dtstartdate As Date = DateValue(cldrdate.Value)
        Dim dtendate As Date = DateAdd(DateInterval.Month, qty, dtstartdate)
        ...
        Dim amount As Double
        Dim monthlyamount As Double = netamt / qty
        _objcmnbLayer._saveDatawithOutParm("DELETE FROM InstallmentTb WHERE TrId = " & trid)
        Dim months As Integer
        emidate = dtstartdate
        For i = 1 To NoOfInstallment
            emidate = DateAdd(DateInterval.Month, NoOfMnth, dtstartdate)
            If emidate > dtendate Then
                emidate = dtendate
            End If
            months = DateDiff(DateInterval.Month, dtstartdate, emidate)
            ...
            _objcmnbLayer._saveDatawithOutParm("Insert into InstallmentTb(trid,installmentdate,amount) values('" & _
                                                      trid & "','" & Format(dtstartdate, "yyyy/MM/dd") & "','" & amount & "')")
            dtstartdate = emidate
        Next
    End Sub
End Class
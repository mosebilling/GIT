...
Public Class WSServiceSalesInvoice
...
#Region "Form Object Declaration"
    ...
    Private WithEvents fCardPackages As SelectCardPackageFrm
#End Region
...
#End Region
    ...
    Public Sub CheckNLoad(Optional ByVal FromTrId As Long = 0)
        ...
        Else
            InvList = _objcmnbLayer._fldDatatable("SELECT TrId FROM ItmInvCmnTb WHERE TrType = 'SIS' AND InvNo = " & Val(numVchrNo.Text) & " AND PreFix = '" & MkDbSrchStr(Trim(txtprefix.Text), True) & "'")
        End If
    ...
    Private Sub ldPostedInv()
        ...
        ItmInvCmnTb = _objcmnbLayer._fldDatatable("SELECT ItmInvCmnTb.*,jobname,cardtypename FROM ItmInvCmnTb " & _
                                                  "LEFT JOIN CardtypemasterTb ON CardtypemasterTb.cardtypeid=ItmInvCmnTb.wsPackageid " & _
                                                  "LEFT JOIN JobTb ON ItmInvCmnTb.[Job Code]=JobTb.jobcode WHERE TrId = " & loadedTrId & " AND TrType = 'SIS'")
        chgbyprg = True
        ActBr = ""
        ...
        txtpackage.Tag = Val(ItmInvCmnTb(0)("wsPackageid") & "")
        txtpackage.Text = Trim(ItmInvCmnTb(0)("cardtypename") & "")
        ...
        getcarddetails(Val(txtCashCustomer.Tag))
        getcardetails(Val(ItmInvCmnTb(0)("carid") & ""))
        'lblservice.Text = Val(lblservice.Text) + 1
        '****************
        cmbcard.Text = ItmInvCmnTb(0)("wscardnumber")
        ...
        sRs = _objcmnbLayer._fldDatatable("SELECT ItmInvTrTb.*, [Item Code],Warrentyname,isSerialNo,fcode,FraCount FROM ItmInvTrTb LEFT JOIN InvItm ON InvItm.ItemId = ItmInvTrTb.ItemId " & _
...
Quit:
    ...
    Private Sub NextNumber()
        ...
        '    dtInv = _objcmnbLayer._fldDatatable("SELECT Prefix,InvNo FROM InvNos WHERE InvType='SIS'")
        ...
        Try
            ...
            With cmbVoucherTp
                If .Items.Count > 0 Then
                    ...
                    cmbVoucherTp.Tag = getvrsId(cmbVoucherTp.Text, 13)
                    getVrsDet(Val(cmbVoucherTp.Tag), "SIS", vrPrefix, vrVoucherNo, vrAccountNo1, vrAccountNo2)
                Else
                    getVrsDet(0, "SIS", vrPrefix, vrVoucherNo, vrAccountNo1, vrAccountNo2)
                End If
            ...
            '    txtCashCustomer.ReadOnly = True
            'Else
            '    txtCashCustomer.ReadOnly = False
            'End If
        Catch ex As Exception
            MsgBox(ex.Message)
        End Try
    ...
    Private Sub txtReference_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtDescr.KeyDown, txtSuppName.KeyDown, txtSuppAlias.KeyDown, txtReference.KeyDown, txtjobname.KeyDown, txtJob.KeyDown, txtCashCustomer.KeyDown
        ...
        ElseIf e.KeyCode = Keys.F2 Then
            If MyCtrl.ReadOnly Then Exit Sub
            If Not fMList Is Nothing Then fMList.Visible = False
...
ctn:
    ...
    Private Sub AddDetails(ByVal DR As DataTable)
        ...
        With grdVoucher
            ...
            End If
            If Val(.Item(ConstDisP, i).Value) > 0 Then
...
nxt:
    ...
    Private Sub btnModify_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnModify.Click
        If btnModify.Text = "&Modify" Then
            ...
            If userType Then
                btnupdate.Tag = IIf(getRight(168, CurrentUser), 1, 0)
                btndelete.Tag = IIf(getRight(169, CurrentUser), 1, 0)
            Else
                btnupdate.Tag = 1
                btndelete.Tag = 1
            End If
    ...
    Private Sub Verify()
        ...
        If Val(txtPurchAlias.Tag) = 0 Then
            ...
            Exit Sub
        End If
        If cmbcard.Text <> "" And Val(lblservice.Text) = 0 Then
            MsgBox("Discount services not available for selected card!" & vbCrLf & "Please renew and continue or remove card", MsgBoxStyle.Exclamation)
            Exit Sub
        End If
        ...
        If cmbcard.Text <> "" And Val(lblservice.Text) = 0 Then
            MsgBox("Discount services not available for selected card!" & vbCrLf & "Please renew and continue or remove the card", MsgBoxStyle.Exclamation)
            Exit Sub
        End If
    ...
    Private Sub saveTrans()
        ...
        If Not isModi Then
chkagain:
            If Not CheckNoExists(txtprefix.Text, Val(numVchrNo.Text), "SIS", "Inventory") Then
        ...
        setInvCmnValue(TrId)
        TrId = Val(_objInv._saveCmn())
        _objcmnbLayer._saveDatawithOutParm("UPDATE ItmInvCmnTb SET carid=" & Val(cmbplatenumber.Tag) & _
                                           ",wscardnumber='" & cmbcard.Text & _
                                           "',CashCustName='" & IIf(txtCashCustomer.Text = "", txtSuppName.Text, txtCashCustomer.Text) & _
                                           "',CashCustid=" & Val(txtCashCustomer.Tag) & _
                                           ",wsPackageid=" & Val(txtpackage.Tag) & _
                                           " WHERE TRID=" & TrId)
        ...
        If isModi = False Then
            ...
            txtprefix.Text = SetNextVrNo(numVchrNo, Val(cmbVoucherTp.Tag), "SIS", "TrType = 'SIS' AND InvNo = ", False, True, True)
        End If
...
deft:
            ...
            setAcctrDetValue(LinkNo, Val(txtSuppAlias.Tag), Trim(txtReference.Text), EntRef, dlAmt, "", Strings.Left(txtJob.Text, 50), 0, 0, "", _
                             "", Val(txtSuppAlias.Tag), Val(txtSuppAlias.Tag) & txtReference.Text, cmbfc.Text, FCRt)
            _objTr.saveAccTrans()
        End If
    ...
    Private Sub setInvDetValue(ByVal Invid As Long, ByVal PPerU As Single, ByVal i As Integer)
        With grdVoucher
            ...
            End If
            _objInv.SlNo = i + 1
            _objInv.TrTypeNo = getVouchernumber("SIS")
    ...
    Private Sub setAcctrCmnValue(ByVal InvTrid As Long, ByVal LinkNo As Long)
        ...
        _objTr.JVTypeNo = getVouchernumber("SIS")
        _objTr.UserId = Trim(CurrentUser & "")
        _objTr.MchName = MACHINENAME
        _objTr.CrtDtTm = Dt
        _objTr.TypeNo = getVouchernumber("SIS")
    ...
    Private Sub SalesOrderFrm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            SetGridHead()
            crtSubVrs(cmbVoucherTp, 13, True)
            lnumFormat = numFormat
            FCRt = 1
            OthCost = 0
            chgbyprg = True
            ...
            Else
                AddNewClick()
                If userType Then
                    btnupdate.Tag = IIf(getRight(167, CurrentUser), 1, 0)
                Else
                    btnupdate.Tag = 1
                End If
...
ChkAgain:
        dtTable = _objcmnbLayer._fldDatatable("SELECT * FROM ItmInvCmnTb WHERE TrType = 'SIS' AND InvNo = " & Val(numVchrNo.Text) & " AND PreFix = '" & MkDbSrchStr(Trim(txtprefix.Text), True) & "'")
        If dtTable.Rows.Count > 0 Then
    ...
    Private Function chkGridvalue() As Boolean
        ...
        With grdVoucher
            ...
            Next
            'dtSrlNo = _objcmnbLayer._fldDatatable("SELECT SerialNo,itemid FROM ItmInvTrTb Left Join ItmInvCmnTb on ItmInvTrTb.Trid=ItmInvCmnTb.trid WHERE TrType='SIS' AND ItmInvCmnTb.Trid<>" & loadedTrId)
...
ter:
        ...
        Try
            ...
            If dt.Rows.Count > 0 Then
                ...
                If Val(txtCashCustomer.Tag) = 0 Then
                    ...
                    dtcust = _objcmnbLayer._fldDatatable("SELECT Add1,Add2,Add3,Phone,CustName,custid FROM CashCustomerTb WHERE customeraccount=" & Val(txtSuppAlias.Tag))
                    If dt.Rows.Count > 0 Then
                        If Trim(dtcust(0)("Add1") & "") <> "" Then txtcustAddress.Text = Trim(dtcust(0)("Add1") & "")
                        If Trim(dtcust(0)("Add2") & "") <> "" Then txtcustAddress.Text = txtcustAddress.Text & vbCrLf & Trim(dtcust(0)("Add2") & "")
                        If Trim(dtcust(0)("Add3") & "") <> "" Then txtcustAddress.Text = txtcustAddress.Text & vbCrLf & Trim(dtcust(0)("Add3") & "")
                        If Trim(dtcust(0)("phone") & "") <> "" Then txtcustAddress.Text = txtcustAddress.Text & vbCrLf & "Ph: " & Trim(dtcust(0)("phone") & "")
                        txtCashCustomer.Text = Trim(dtcust(0)("CustName") & "")
                        txtCashCustomer.Tag = Val(dtcust(0)("custid") & "")
                        getcarddetails(Val(txtCashCustomer.Tag))
                    End If
                End If
                If accid = 0 Then
                ...
                Dim dtAcctrid As DataTable
                Dim linkno As Long = 0
                If loadedTrId > 0 Then
                    dtAcctrid = _objcmnbLayer._fldDatatable("Select linkno from AccTrCmn left join ItmInvCmnTb on ItmInvCmnTb.trid=AccTrCmn.lnkno where lnkno=" & loadedTrId)
                    If dtAcctrid.Rows.Count > 0 Then
                        linkno = dtAcctrid(0)("linkno")
                    End If
                End If

                Dim iNBal As Double = getAccBal(Val(txtSuppAlias.Tag), linkno)
    ...
    Private Sub LoadReport(ByVal FileName As String, ByVal RptCaption As String, Optional ByVal ToPrint As Boolean = False, Optional ByVal pno As Integer = 0)
        ...
        Dim ds As DataSet
        ds = _objInv.ldInvoice("rturnWSServiceDetailsForInvoicePrint", pno)
        If ToPrint Then
...
u:
    ...
    Private Sub grdVoucher_CellEndEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdVoucher.CellEndEdit
        ...
        If col = ConstQty Or col = ConstUPrice Or col = ConstDisAmt Or col = ConstLTotal Or col = ConstTaxP Or col = ConstDisP Or col = ConstEndReading Then
    ...
    Private Sub btnprint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnprint.Click
        If enableDuplicateBill Then
            ...
            Dim dt As DataTable
            dt = _objcmnbLayer._fldDatatable("SELECT case when Printed='true' then 1 else 0 end p from ItmInvCmnTb where  trtype='SIS'  and  InvNo=" & Val(numPrintVchr.Text) & " and  PreFix='" & txtPPrefix.Text & "'")
            If dt.Rows.Count > 0 Then
                If dt(0)(0) = 0 Then
                    PrepareRpt("SIS", True)
                End If
            ...
            If frm.chkduplicate.Checked Then
                PrepareRpt("SIS", True, 1)
            End If
            If frm.chktriplicate.Checked Then
                PrepareRpt("SIS", True, 2)
            End If
        Else
            PrepareRpt("SIS", True)
        End If
    ...
    Private Sub fCashCust_selectcust(ByVal custname As String, ByVal custaddress As String, ByVal Cashcustid As Long, ByVal cardnumber As String, ByVal phone As String, ByVal gstn As String) Handles fCashCust.selectcust
        ...
        getcarddetails(Cashcustid)
        cmbcard.Text = cardnumber
    End Sub
    Private Sub getcarddetails(ByVal Cashcustid As Long)
        Dim dt As DataTable
        dt = _objcmnbLayer._fldDatatable("SELECT SerialNo from ItmInvCmnTb Left join ItmInvTrTb on ItmInvTrTb.trid=ItmInvCmnTb.trid where CashCustid=" & Cashcustid & " group by SerialNo")
        Dim i As Integer
        cmbcard.Items.Clear()
        For i = 0 To dt.Rows.Count - 1
            cmbcard.Items.Add(dt(i)("SerialNo"))
        Next
        'cmbcard.Items.Add("")
        If cmbcard.Items.Count > 1 Then cmbcard.SelectedIndex = 1
        dt = _objcmnbLayer._fldDatatable("SELECT platenumber from CarMasterTb where customerid=" & Cashcustid)
        cmbplatenumber.Items.Clear()
        For i = 0 To dt.Rows.Count - 1
            cmbplatenumber.Items.Add(dt(i)("platenumber"))
        Next
        dt = _objcmnbLayer._fldDatatable("select Alias,AccDescr,accid from AccMast LEFT JOIN CashCustomerTb ON CashCustomerTb.customeraccount=AccMast.accid where custid=" & Val(txtCashCustomer.Tag))
        If dt.Rows.Count > 0 Then
            chgbyprg = True
            txtSuppAlias.Text = dt(0)("Alias")
            txtSuppAlias.Tag = dt(0)("accid")
    ...
    Private Sub cmbcard_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cmbcard.KeyDown
        If e.KeyCode = Keys.Return Then
            cmbplatenumber.Focus()
        End If
    End Sub
    Private Sub loadcarddetails(ByVal isloaditems As Boolean)
        Dim dt As DataTable
        Dim i As Integer
        Dim dtItms As DataTable
        Dim found As Boolean
        ...
        dt = _objvehicle.returnWSCardHistory(1, cmbcard.Text, loadedTrId, Val(txtpackage.Tag))
        If dt.Rows.Count > 0 Then
            ...
            End If
        End If
        If cmbcard.Text <> "" And Val(lblservice.Text) = 0 And isloaditems Then
            MsgBox("Discount services not available for selected card!" & vbCrLf & "Please renew and continue or remove the card", MsgBoxStyle.Exclamation)
            'Exit Sub
        End If
        'dt = _objcmnbLayer._fldDatatable("select itemid,discpercentage from CardItemTb inner join (select  disccardid from ItmInvTrTb where SerialNo='" & cmbcard.Text & "' group by SerialNo,disccardid) qrycardtype on CardItemTb.cardtypeid=qrycardtype.disccardid")
        If isloaditems Then
            dt = _objcmnbLayer._fldDatatable("select itemid,discpercentage from CardItemTb where cardtypeid =" & Val(txtpackage.Tag))
            grdVoucher.Rows.Clear()
            With grdVoucher
                For i = 0 To dt.Rows.Count - 1
                    dtItms = getItmDtls(4, Val(dt(i)("itemid")), True)
                    If dtItms.Rows.Count > 0 Then
                        found = True
                        .Rows.Add()
                        RowIndex = .RowCount - 1
                        .CurrentCell = .Item(ConstItemCode, RowIndex)
                        If Val(lblservice.Text) > 0 Then
                            .Item(ConstDisP, RowIndex).Value = Format(dt(i)("discpercentage"), lnumFormat)
                        End If
    ...
    Private Sub savecarmaster()
        Dim dt As DataTable
        If Val(cmbplatenumber.Tag) = 0 Then
            dt = _objcmnbLayer._fldDatatable("Select carid from CarMasterTb where platenumber='" & cmbplatenumber.Text & "'")
            If dt.Rows.Count > 0 Then
                cmbplatenumber.Tag = dt(0)("carid")
                Exit Sub
            End If
        End If
        Dim _objvechicle As New clsVechicle
        If Val(txtCashCustomer.Tag) > 0 Then
            _objvechicle.cartype = txtvtype.Text
            _objvechicle.platenumber = cmbplatenumber.Text
            _objvechicle.regyear = txtregyear.Text
            _objvechicle.chassisnumber = ""
            _objvechicle.bodynumber = ""
            _objvechicle.carid = Val(cmbplatenumber.Tag)
            _objvechicle.customerid = Val(txtCashCustomer.Tag)
            cmbplatenumber.Tag = _objvechicle.savecarmaster()
        End If

    End Sub

    Private Sub cmbplatenumber_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cmbplatenumber.KeyDown, txtvtype.KeyDown, txtregyear.KeyDown, txtotherdetails.KeyDown
        If e.KeyCode = Keys.Return Then
            SendKeys.Send("{TAB}")
        End If
    End Sub

    Private Sub cmbplatenumber_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbplatenumber.SelectedIndexChanged
        getcardetails()
    End Sub
    Private Sub getcardetails(Optional ByVal carid As Integer = 0)
        Dim dt As DataTable
        If carid = 0 Then
            dt = _objcmnbLayer._fldDatatable("Select carid,platenumber,cartype,regyear,chaisenumber,bodynumber,remarks from CarMasterTb where platenumber='" & cmbplatenumber.Text & "'")
        Else
            dt = _objcmnbLayer._fldDatatable("Select carid,platenumber,cartype,regyear,chaisenumber,bodynumber,remarks from CarMasterTb where carid='" & carid & "'")
        End If
        If dt.Rows.Count > 0 Then
            cmbplatenumber.Tag = dt(0)("carid")
            cmbplatenumber.Text = dt(0)("platenumber")
            txtvtype.Text = dt(0)("cartype")
            txtregyear.Text = dt(0)("regyear")
            txtotherdetails.Text = Trim(dt(0)("remarks") & "")
        Else
            cmbplatenumber.Tag = ""
            cmbplatenumber.Text = ""
            txtvtype.Text = ""
    ...
    Private Sub cmbplatenumber_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbplatenumber.Validated
        Dim dt As DataTable
        dt = _objcmnbLayer._fldDatatable("Select carid,platenumber,cartype,regyear,chaisenumber,bodynumber,remarks from CarMasterTb where platenumber='" & cmbplatenumber.Text & "'")
        If dt.Rows.Count > 0 Then
            cmbplatenumber.Tag = dt(0)("carid")
    ...
    Private Sub txtpackage_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtpackage.KeyDown
        If cmbcard.Text = "" Then
            MsgBox("Select valid card", MsgBoxStyle.Exclamation)
            cmbcard.Focus()
            Exit Sub
        End If
        If e.KeyCode = Keys.F2 Then
            If Not fCardPackages Is Nothing Then
                fCardPackages.Close()
                fCardPackages = Nothing
            End If
            fCardPackages = New SelectCardPackageFrm
            With fCardPackages
                .loadcardPackage(cmbcard.Text)
                .ShowDialog()
            End With
    ...
    Private Sub fCardPackages_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles fCardPackages.FormClosed
        fCardPackages = Nothing
    End Sub


    Private Sub txtpackage_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtpackage.Validated
        If txtpackage.Text = "" Then
            ...
            With grdVoucher
                For i = 0 To .RowCount - 1
                    ...
                    .Item(ConstDisAmt, i).Value = Format(((CDbl(.Item(ConstActualPrice, i).Value) * Val(.Item(ConstDisP, i).Value)) / 100) * CDbl(.Item(ConstQty, i).Value), lnumFormat)
                    If EnableGST Then getGSTDetails(Trim(.Item(ConstBarcode, i).Value & ""), i, True)
                Next
            End With
            calculate()
        End If
    ...
    Private Sub fCardPackages_selectPackage(ByVal packageid As Integer, ByVal packagename As String, ByVal remainingServices As Integer) Handles fCardPackages.selectPackage
        txtpackage.Text = packagename
        txtpackage.Tag = packageid
        lblservice.Text = remainingServices

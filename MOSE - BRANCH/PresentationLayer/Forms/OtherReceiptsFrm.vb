...
#End Region
    ...
    Public TmpleAmt As Double
    Public TmpleDate As Date
    Public jobCustomer As Long
    Private donotload As Boolean
    Public reference As String
    Public rvamt As Double
    Public closeonSave As Boolean
    ...
    Private Sub btnexit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnexit.Click
        ...
        Me.Close()
    End Sub
    Private Sub setLastPaiddateOnTempleAdmission()
        _objcmnbLayer._saveDatawithOutParm("UPDATE TempleMembershipTb SET LastPaidDate='" & Format(TmpleDate, "yyyy/MM/dd") & "' WHERE MemberCode='" & txtjobcode.Text & "'")
    End Sub
    Private Sub AddRow()
        With grdpayment
        ...
        End With
    End Sub
    Private Sub AddRVRow()
        With grdVoucher
            activecontrolname = "grdVoucher"
            .Rows.Add(1)
            .CurrentCell = .Item(ConstAlias, .RowCount - 1)
            If isTemple Then
                .Item(ConstReference, .RowCount - 1).Value = txtjobcode.Text
            End If
...
ctn:
        ...
        Try
            ...
            If dtTable.Rows.Count > 0 Then
                With grdpayment
                    ...
                    lblaccountbalance.Text = getAccBal(Val(dtTable(0)("accid")))
    ...
    Private Function isGridValidCredit() As Boolean
        ...
        Return True
    End Function
    Private Sub makeClear(Optional ByVal skipselectchange As Boolean = False)
        ...
        chgbyprg = False
        dtpdate.Focus()
        If Not skipselectchange Then cmbVoucherTp_SelectedIndexChanged(cmbVoucherTp, New System.EventArgs)
        assaignTotal()
        cmbdeliveredBy.Text = ""
    End Sub
...
chkagain:
        ...
        With grdVoucher
            For i = 0 To .RowCount - 1
                ...
                setAcctrDetValue(LinkNo, i, grdVoucher, 0)
                '_objTr.saveAccTrans()
nxt:
            Next
        End With
...
nxt1:
        ...
        If isTemple And Not isModi Then
            setLastPaiddateOnTempleAdmission()
        End If
        numPrintVchr.Text = numVchrNo.Text
        txtprintprefix.Text = txtPreFix.Text
        If Not isModi Then
            'txtPreFix.Text = SetNextVrNo(numVchrNo, Val(cmbVoucherTp.Tag), "RV", "JvType = 'RV' AND JvNum = ", True, True, True)
        Else
        ...
        MsgBox("Receipt Voucher # " & numVchrNo.Text & " Saved Successfully", MsgBoxStyle.Information)
        If closeonSave Then
            Me.Close()
        End If
    End Function
    Private Sub setAcctrCmnValue()
        ...
        _objTr.OthInf = txtreceivedfrom.Text
    End Sub
    Private Sub setAcctrDetValue(ByVal lnkNo As Long, ByVal _row As Integer, ByVal grd As DataGridView, ByVal trtype As Integer)
        ...
        If isTemple Then
            dtrow("JobCode") = ""
        Else
            dtrow("JobCode") = IIf(Trim(grd.Item(ConstJob, _row).Value & "") = "", Trim(txtjobcode.Text & ""), Trim(grd.Item(ConstJob, _row).Value & ""))
        End If
...
nxt:
    ...
    Private Sub btnPreview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPreview.Click
        If TabControl1.SelectedIndex = 0 Then
            If chkFormat.Checked Then
                ...
                fRptFormat.RptType = IIf(chktraditional.Checked, "RVT", "RV")
                fRptFormat.ShowDialog()
                fRptFormat = Nothing
            Else
                PrepareRpt(IIf(chktraditional.Checked, "RVT", "RV"))
            End If
        Else
            fRptFormat = New RptFormatfrm
    ...
    Private Sub btndelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btndelete.Click
        ...
        If isTemple Then Me.Close() : Exit Sub
        btnModify_Click(btnModify, New System.EventArgs)
    End Sub
    ...
    Private Sub txtBank_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtBank.Validated
        ...
        End If
    End Sub
    Public Sub editRecord(Optional ByVal linkno As Long = 0, Optional ByVal isfromJob As Boolean = False)
        ...
        Else
            loadedTrId = linkno
        End If
        If grdVoucher.ColumnCount = 0 Then SetGridHeadExpense()
        If grdpayment.ColumnCount = 0 Then SetGridHeadPayment()
        If Not userType Then
            btndelete.Tag = 1
            btnupdate.Tag = 1
        Else
    ...
    Private Sub ldRec()
        ...
        Dim drqry As String
        drqry = getAccTrDetbyloadedTrId(loadedTrId, " AND DealAmt>0", True)
        ...
        qry = qry & " " & getAccTrDetbyloadedTrId(loadedTrId, " AND DealAmt<0")
        qry = qry & " " & drqry
        ...
        If dtAccTrCmn.Rows.Count > 0 Then
            ...
            txtreceivedfrom.Text = dtAccTrCmn(0)("OthInf")
    ...
    Private Sub loadCredit(ByVal dtAccTr As DataTable)
        ...
        If grdpayment.CurrentRow Is Nothing Then
            FindNextCell(grdVoucher, 0, 1)
        Else
    ...
    End Sub
    Private Sub SetGridHeadExpense()
        With grdVoucher
    ...
    Private Sub OtherReceiptsFrm_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        dtpdate.Focus()
    End Sub
    ...
    Private Sub grdpayment_CellValidated(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdpayment.CellValidated
        ...
        SrchText = ""
    End Sub
    Public Sub Valid(ByVal RowIndex As Integer, ByVal ColIndex As Integer, ByVal grd As DataGridView)
    ...
    Private Sub Showfmlist()
        If Not _srchOnce Then
            chgbyprg = True
            Dim PopupLoc As Point
            Dim x As Integer = Me.Width - plsrch.Width - (100 + IIf(_srchTxtId = 4 Or _srchTxtId = 5, 100, 0))
                    ...
                    If FindNextCell(grdpayment, grdpayment.CurrentCell.RowIndex, grdpayment.CurrentCell.ColumnIndex + 1) Then
                        If grdpayment.RowCount = 1 Then
                            ...
                            If grdVoucher.RowCount = 0 Then
                                ...
                                grdVoucher.Item(ConstPAmount, 0).Value = CDbl(lblTlDebit.Text)
                                assaignTotal()
                            Else
                                SrchText = ""
                                activecontrolname = "grdVoucher"
...
nxt:
    ...
    Private Sub OtherReceiptsFrm_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
    ...
    End Sub
    Private Sub OtherReceiptsFrm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        formload(donotload)
        donotload = False
        lnumformat = numFormat
        chgbyprg = True
        ...
        NDec = NoOfDecimal
        Timer1.Enabled = True
        If txtjobcode.Text <> "" Or editlinkno > 0 Then
            Timer2.Enabled = True
        End If
    End Sub
    Private Sub formload(ByVal donotload As Boolean)
        If donotload = False Then
            SetGridHeadPayment()
            SetGridHeadExpense()
        End If
    End Sub
    ...
    Public Sub ldjbname()
        Dim chkDatatable As DataTable
        If Not isTemple Then
            chgbyprg = True
            lbljobname.Text = (_objcmnbLayer.isValidEntry(txtjobcode.Text, 3))
            If jobCustomer > 0 Then
                AddRVRow()
                chkDatatable = EntriesValidation(jobCustomer, 9)
                If chkDatatable.Rows.Count > 0 Then
                    With grdVoucher
                        .Item(ConstAlias, .CurrentCell.RowIndex).Value = SrchText
                        If reference <> "" Then
                            .Item(ConstReference, .CurrentCell.RowIndex).Value = reference
                        End If
                        ...
                        .Item(ConstBrId, .CurrentCell.RowIndex).Value = ""
                        .Item(ConstGrpSetOn, .CurrentCell.RowIndex).Value = GrpSetOn(chkDatatable(0)("AccountNo"))
                        .Item(ConstDescr, .CurrentCell.RowIndex).Value = "AMOUNT RECEIVED AGAINST " & txtjobcode.Text
                        If rvamt > 0 Then
                            .Item(ConstPAmount, .CurrentCell.RowIndex).Value = rvamt
                        End If
                    End With
                    grdpayment.Item(ConstDescr, grdpayment.CurrentCell.RowIndex).Value = "AMOUNT RECEIVED FROM " & chkDatatable(0)("AccDescr")
                    If rvamt > 0 Then
                        grdpayment.Item(ConstPAmount, grdpayment.CurrentCell.RowIndex).Value = rvamt
                    End If
                    grdpayment.Select()
                    grdpayment.CurrentCell = grdpayment.Item(ConstPAmount, 0)
                    activecontrolname = "grdpayment"
                    grdpayment.BeginEdit(True)
                End If
            End If
        ...
        Else
            ...
            lbljobname.Text = (_objcmnbLayer.isValidEntry(txtjobcode.Text, 13))
            If TmpleAmt > 0 Then
                AddRVRow()
                grdVoucher.Item(ConstPAmount, grdVoucher.RowCount - 1).Value = Format(TmpleAmt, numFormat)
                grdVoucher.Item(ConstDescr, grdVoucher.RowCount - 1).Value = "YEARLY AMOUNT RECEIVED FROM " & lbljobname.Text
                If grdpayment.RowCount > 0 Then
                    grdpayment.Item(ConstPAmount, grdpayment.RowCount - 1).Value = Format(TmpleAmt, numFormat)
                    grdpayment.Item(ConstDescr, grdpayment.RowCount - 1).Value = "YEARLY AMOUNT RECEIVED FROM " & lbljobname.Text
                End If
                assaignTotal()
            End If

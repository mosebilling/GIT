...
Public Class TreatmentItem
    ...
    Private Sub QuickItem_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Label21.Visible = EnableGST
        txtpriceWtax.Visible = EnableGST
        Label19.Visible = EnableGST
        txthsncode.Visible = EnableGST
        txthsncode.Visible = EnableGST
        lblgstp.Visible = enableGCC
        Timer1.Enabled = True
    End Sub
...
Err:
    ...
    Private Sub setValue()
        ...
        If Val(numunitprice.Text) = 0 Then numunitprice.Text = Format(0, numFormat)
        _objItmMast.salesPrice = CDbl(numunitprice.Text)
        _objItmMast.Category = "Treatment"
        ...
        Else
            ...
            _objItmMast.LstModiDt = Date.Now
        End If
        _objItmMast.Ismodi = IIf(ItemId > 0, True, False)
    End Sub
    ...
    Private Sub loadItemdetails()
        Dim dt As DataTable
        dt = _objcmnbLayer._fldDatatable("SELECT [Item Code],Description,UnitPrice [Amount],(((isnull(IGST,0)+ISNULL(vat,0)+isnull(rgcess,0))*UnitPrice)/100)+UnitPrice+isnull(additionalcess,0) [Tax Amount],ItemID FROM InvItm " & _
                                         "LEFT JOIN GSTTb ON GSTTb.HSNCode=InvItm.HSNCode " & _
        ...
        With grdItem
            ...
            .Columns("Tax Amount").Visible = EnableGST
        End With
    End Sub
    ...
    Private Sub calculateTaxFromUnitPrice(ByVal fromUnitpice As Boolean)
        ...
        If Val(numunitprice.Text) = 0 Then numunitprice.Text = Format(0, numFormat)
        If Val(txtpriceWtax.Text) = 0 Then txtpriceWtax.Text = Format(0, numFormat)
        ...
        Else
            txtpriceWtax.Text = Format(CDbl(numunitprice.Text) + ((CDbl(numunitprice.Text) * CDbl(txtpriceWtax.Tag)) / 100), numFormat)
        End If
        chgbyprgAmt = False
    End Sub
    ...
    Private Sub grdItem_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles grdItem.DoubleClick
        With grdItem
            If .RowCount > 0 Then
                Dim itemid As Long
                itemid = .Item("itemid", .CurrentRow.Index).Value
                ldRec(itemid)
            End If
        End With
    End Sub

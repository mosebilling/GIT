...
Public Class CreateAccNew
    ...
    Private Sub ldAccounts(ByVal AccNo As Long)
        ...
        cmbarea.Text = Trim("" & dtTable(0)("AreaCode"))
        cmbpricegroup.SelectedIndex = Val("" & dtTable(0)("pricegroup"))
        If Not IsDBNull(dtTable(0)("iscashinvoiceonly")) Then
            chkcashinvoice.Checked = dtTable(0)("iscashinvoiceonly")
        Else
            chkcashinvoice.Checked = False
        End If
        Chgamt = False
...
ext:
    ...
    Private Sub cmdOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdOk.Click
        updateClick()
    End Sub
    Private Sub updateClick(Optional ByVal donotclear As Boolean = False)
    ...
    Private Function ModiAcc() As Boolean
        ...
        _objcmnbLayer._saveDatawithOutParm("UPDATE AccMast SET Alias = '" & MkDbSrchStr(txtRec0.Text) & "', AccDescr = '" & _
                                           ...
                                           " AreaCode='" & cmbarea.Text & "'," & _
                                           " iscashinvoiceonly = " & IIf(chkcashinvoice.Checked, 1, 0) & "," & _
                                           " pricegroup=" & cmbpricegroup.SelectedIndex & _
                                           " WHERE accid = " & accid)
    ...
    Private Function saveAcc() As Boolean
        ...
        If Not isModi Then
            ...
            If Val(txtlimit.Text) = 0 Then txtlimit.Text = 0
            If Val(txtduedays.Text) = 0 Then txtduedays.Text = 0
            _objcmnbLayer._saveDatawithOutParm("INSERT INTO AccMast (AccountNo, Alias, AccDescr, S1AccId,OpnBal,CreditLimit,DueDays,CountryCode,isTaxRegistered,placesupply,SlsmanId,BranchId,AreaCode,pricegroup,iscashinvoiceonly) VALUES (" & _
                                               ...
                                               IIf(chkregistered.Checked, 1, 0) & "," & cmbplaceofsupply.SelectedIndex & ",'" & cmbsalesman.Text & "','" & cmbBr.Text & "','" & cmbarea.Text & "'," & cmbpricegroup.SelectedIndex & "," & IIf(chkcashinvoice.Checked, 1, 0) & ")")
        End If
    End Function
    ...
    End Sub
    Private Sub ldStateBranch()
        'Dim ds As DataSet = _objcmnbLayer._ldDataset("SELECT statecode FROM StateMasterTb " & _
        '"SELECT Branchcode FROM BranchTb", False)
        ...
        For i = 0 To ds.Tables(1).Rows.Count - 1
            cmbstate.Items.Add(ds.Tables(1)(i)("statecode"))
        Next
        '_fcTable = ds.Tables(2)
        _slsManTable = ds.Tables(3)
    ...
    Private Sub CreateAccNew_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Leave
    ...
    Private Sub CreateAccNew_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If iscust Then
            ...
            End If
            chkcashcustomer.Text = "Create Cash Customer"
            cmbarea.Visible = True
            Label23.Visible = True
            cmbarea.Items.Clear()
            AddtoCombo(cmbarea, "select areacode from areatb", True)
        Else
            'ldSupplier()
        ...
        Try
            ...
            If Val(txtAddr1.Tag) = 1 Then
                _objcmnbLayer._saveDatawithOutParm("INSERT INTO AccMastAddr (AccountNo,Address1,Address2,Address3,Address4,Phone,Fax,ContactName,EMail,Website," & _
                                                                "TrdLcno,TrdDate,GSTIN,Remarks,PhoneAltr) VALUES(" & _
                                                                   Val(accid) & ",'" & _
    ...
    Private Sub saveCustomer(ByVal accid As Integer)
        ...
        With _objInv
            ...
            .issupp = Not iscust
            .GSTN = IIf(enableGCC, txtlcno.Text, txtgstin.Text)
            .saveCashCustomer()
        End With
    End Sub
    ...
    '    brcodition = IIf(Condition = "", "", "WHERE " & Condition) & IIf(UsrBr = "", "", " AND (BranchId='" & UsrBr & "' Or isnull(BranchId,'')='') ")
    '    _vtable = _objcmnbLayer._fldDatatable("SELECT Alias,AccDescr,isnull(Bal,0)+OpnBal [Closing Bal],OpnBal [Opening Bal],Phone,ContactName,EMail,AccMast.AccountNo FROM AccMast " & _
    '                                           "left Join BalanceQr On BalanceQr.AccountNo=AccMAst.accid " & _
    '                                          "LEFT JOIN AccMastAddr ON AccMast.accid=AccMastAddr.AccountNo left join S1AccHd on AccMast.S1AccId=S1AccHd.S1AccId " & brcodition & " ORDER BY Descr")
    '    grdItem.DataSource = _vtable
    '    SetGridSupplier()
    '    setComboGrid()
    'End Sub
    Private Sub ldLedger()
        Dim brcodition As String
        brcodition = IIf(Condition = "", "", "WHERE " & Condition) & IIf(UsrBr = "", "", " AND (BranchId='" & UsrBr & "' Or isnull(BranchId,'')='') ")
        Dim balancquery As String
        balancquery = "select * from  BalanceQr"
        'If UsrBr = "" Then

        'Else
        '    balancquery = "select * from  BalanceQr left join accmast on BalanceQr.accountno=accmast.accid" & _
        '                  " where isnull(CmnBrId,'') in ('','" & UsrBr & "')"
        'End If
        _vtable = _objcmnbLayer._fldDatatable("SELECT Alias,AccDescr,isnull(Bal,0)+OpnBal [Closing Bal],OpnBal [Opening Bal],Phone,ContactName,EMail,BranchId,AccMast.AccountNo,accid FROM AccMast " & _
                                                "left Join (" & balancquery & ")BalanceQr On BalanceQr.AccountNo=AccMAst.accid " & _
                                              "LEFT JOIN AccMastAddr ON AccMast.accid=AccMastAddr.AccountNo left join S1AccHd on AccMast.S1AccId=S1AccHd.S1AccId " & brcodition & " ORDER BY Descr")
        grdItem.DataSource = _vtable
        SetGridCustomer()
        setComboGrid()
    ...
    Private Sub btnremove_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnremove.Click
        ...
        dt = _objcmnbLayer._fldDatatable("SELECT TrId FROM ItmInvCmnTb WHERE CSCode=" & accid & _
                                         ...
                                         " UNION ALL SELECT Linkno FROM AccTrDet WHERE AccountNo=" & accid & _
                                         "UNION ALL SELECT invoicenumber FROM AmcCustomerTb WHERE customerid =" & accid)
        If dt.Rows.Count > 0 Then
            MsgBox("Code #" & txtRec0.Text & " having transactions, You can't remove record", MsgBoxStyle.Exclamation)
            Exit Sub
        End If
        If MsgBox("You are going to remove the Party # " & txtRec0.Text & " # Permenantly! Are you sure?", MsgBoxStyle.Question + MsgBoxStyle.YesNo) = MsgBoxResult.No Then
            Exit Sub
        End If
    ...
    Sub SetGridHead()
        With grdVoucher
            ...
            cmb = New DataGridViewComboBoxColumn
            cmb.Items.Add("")
            If Not _fcTable Is Nothing Then
    ...
    Public Sub BeginEdit()
        ...
        ChgByPrg = False
    End Sub
    Public Sub BeginEditPriceList()
        ChgByPrg = True
        With grdpricelist
            If .RowCount = 0 Then Exit Sub
            activecontrolname = "grdpricelist"
            .BeginEdit(True)
        End With
        ChgByPrg = False
    End Sub
    Protected Overrides Function ProcessCmdKey(ByRef msg As System.Windows.Forms.Message, ByVal keyData As System.Windows.Forms.Keys) As Boolean
        Try
            ...
            End If
            If msg.Msg = WM_KEYDOWN Or msg.Msg = WM_KEYUP Then
                If activecontrolname = "grdpricelist" Then
                    If msg.WParam.ToInt32() = CInt(Keys.Down) Or msg.WParam.ToInt32() = CInt(Keys.Up) Or msg.WParam.ToInt32() = CInt(Keys.Enter) Or msg.WParam.ToInt32() = CInt(Keys.F2) Or msg.WParam.ToInt32() = CInt(Keys.F3) Or msg.WParam.ToInt32() = CInt(Keys.F4) Then
                        grdpricelist_KeyDown(Nothing, New KeyEventArgs(keyData))
                        Return True
                    End If
                End If
            End If
    ...
    Private Sub grdVoucher_EditingControlShowing(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewEditingControlShowingEventArgs) Handles grdVoucher.EditingControlShowing
        ...
        ElseIf Col = ConstTrdate Then
            Dim tb1 As CalendarEditingControl = CType(e.Control, CalendarEditingControl)
            RemoveHandler tb1.TextChanged, AddressOf calender_TextChanged
            AddHandler tb1.TextChanged, AddressOf calender_TextChanged
        End If
    End Sub
    ...
    End Sub
    Private Sub calender_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        If ChgByPrg Then Exit Sub
        cmdOk.Enabled = True
        Try
            Dim col As Integer
            col = grdVoucher.CurrentCell.ColumnIndex
            Dim MyCtrl As CalendarEditingControl = sender
            If ChgByPrg Then Exit Sub
            If col = ConstTrdate Then
                With grdVoucher
                    .Item(ConstTrdate, .CurrentRow.Index).Value = MyCtrl.Text
                End With
            End If
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub numopnBal_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles numopnBal.KeyPress, txtlimit.KeyPress, txtduedays.KeyPress, txtdiscount.KeyPress
    ...
    Private Sub btnpricelistclose_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnpricelistclose.Click
        plpricelist.Visible = False
    End Sub

    Private Sub btnpricelist_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnpricelist.Click
        If accid = 0 Then
            If MsgBox("Customer Details not saved! Do you want save & Continue?", MsgBoxStyle.YesNo + MsgBoxStyle.Question) = MsgBoxResult.No Then Exit Sub
            updateClick(True)
        End If
        plpricelist.Visible = True
        plpricelist.Top = Panel1.Top
        plpricelist.Left = Panel4.Left
        plpricelist.Width = Panel4.Width
        plpricelist.Height = TabPage1.Height - 10
        SetLevelGrid()
        FillPriceList()
    ...
    Private Sub FillPriceList()
        ...
        strStockCondition = " WHERE isnull(ishide,0)=0   "
        Dim lvl As String = ""
        Dim i As Integer
        ...
        Dim query As String
        query = "SELECT '' as Tag,[Item Code] ,InvItm.Description,Unit,UnitPrice,isnull(tr.discount,0) Discount,invitm.itemid,isnull(tr.id,0)id,isnull(customerprice,0)customerprice from invitm " & _
        " LEFT JOIN (Select itemid,discount,id from CustomerWisePriceDiscountTb where customerid=33)pricelist on invitm.itemid=pricelist.itemid " & _
        " LEFT JOIN (SELECT GrpItmCode Lvl1,UnqGrpId LvlId1 from GrpItmTb) GrpItmTb1 ON INVITM.Level1=GrpItmTb1.LvlId1 " & _
        " LEFT JOIN (SELECT GrpItmCode Lvl2,UnqGrpId LvlId2 from GrpItmTb) GrpItmTb2 ON INVITM.Level2=GrpItmTb2.LvlId2 " & _
        " LEFT JOIN (SELECT GrpItmCode Lvl3,UnqGrpId LvlId3 from GrpItmTb) GrpItmTb3 ON INVITM.Level3=GrpItmTb3.LvlId3 " & _
        " LEFT JOIN (SELECT GrpItmCode Lvl4,UnqGrpId LvlId4 from GrpItmTb) GrpItmTb4 ON INVITM.Level4=GrpItmTb4.LvlId4 " & _
        " LEFT JOIN (SELECT GrpItmCode Lvl5,UnqGrpId LvlId5 from GrpItmTb) GrpItmTb5 ON INVITM.Level5=GrpItmTb5.LvlId5 " & _
        " LEFT JOIN (SELECT GrpItmCode Lvl6,UnqGrpId LvlId6 from GrpItmTb) GrpItmTb6 ON INVITM.Level6=GrpItmTb6.LvlId6 " & _
        " LEFT JOIN (SELECT GrpItmCode Lvl7,UnqGrpId LvlId7 from GrpItmTb) GrpItmTb7 ON INVITM.Level7=GrpItmTb7.LvlId7 " & _
        " LEFT JOIN (SELECT GrpItmCode Lvl8,UnqGrpId LvlId8 from GrpItmTb) GrpItmTb8 ON INVITM.Level8=GrpItmTb8.LvlId8 " & _
        " LEFT JOIN (SELECT GrpItmCode Lvl9,UnqGrpId LvlId9 from GrpItmTb) GrpItmTb9 ON INVITM.Level9=GrpItmTb9.LvlId9 " & _
        " LEFT JOIN (SELECT GrpItmCode Lvl10,UnqGrpId LvlId10 from GrpItmTb) GrpItmTb10 ON INVITM.Level10=GrpItmTb10.LvlId10 " & _
        " LEFT JOIN (select customerprice,customerid,itemid,discount,id from CustomerWisePriceDiscountTb where customerid = '" & accid & "' )tr on  INVITM.itemid=tr.itemid" & strStockCondition & " order by itemlistorder"

        dtTable = _objcmnbLayer._fldDatatable(query)
        grdpricelist.DataSource = dtTable

        SetGridHeadPriceList()
        Me.Cursor = Cursors.Default
    End Sub
    Private Sub SetGridHeadPriceList()
        SetGridProperty(grdpricelist)
        With grdpricelist
            .ReadOnly = False
            Dim i As Integer
            For i = 0 To .ColumnCount - 1
                If .Columns(i).HeaderText <> "Discount" Then
                ...
                End If
            Next
            .Columns.Item("Tag").HeaderText = "Tag"
            .Columns.Item("Tag").Visible = False
            .Columns.Item("Tag").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter

            .Columns.Item("Item Code").Width = 100

            '.Columns.Item("Unit").HeaderText = "Tag"
            .Columns.Item("Unit").Width = 40
            ...
            .Columns("Discount").ReadOnly = False
            ...
            .Columns("customerprice").HeaderText = "Customer Price"
            .Columns("customerprice").ReadOnly = False
            .Columns("customerprice").DefaultCellStyle.Format = "N" & NoOfDecimal
            .Columns("customerprice").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
            .Columns("customerprice").Width = 100
            ...
            resizeGridColumn(grdpricelist, 2)
            ChgByPrg = True
            cmbpriceheader.Items.Clear()
            For i = 0 To .ColumnCount - 1
                cmbpriceheader.Items.Add(.Columns(i).HeaderText)
            Next
            If cmbpriceheader.Items.Count > 0 Then cmbpriceheader.SelectedIndex = 2
            ChgByPrg = False
        End With
        
    End Sub

    Private Sub grdpricelist_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdpricelist.CellClick
        Dim intColumnIndex As Integer = 0
        If e.ColumnIndex = 0 Then
            checkOrUncheckTag(grdpricelist, e, intColumnIndex)
        End If

        ''grdpricelist.SelectionMode = DataGridViewSelectionMode.CellSelect
    End Sub

    Private Sub grdpricelist_CellEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdpricelist.CellEnter
        ChgByPrg = True
        grdpricelist.BeginEdit(True)
        ChgByPrg = False
    End Sub


    Private Sub grdpricelist_DataError(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles grdpricelist.DataError

    End Sub

    Private Sub grdpricelist_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles grdpricelist.Enter
        activecontrolname = "grdpricelist"

    End Sub

    Private Sub grdpricelist_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles grdpricelist.GotFocus
        activecontrolname = "grdpricelist"
    End Sub

    Private Sub grdpricelist_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles grdpricelist.KeyDown
        Try

            If e.KeyCode = Keys.Enter Then
                If grdpricelist.RowCount = 0 Then Exit Sub
                FindNextCell(grdpricelist, grdpricelist.CurrentCell.RowIndex, grdpricelist.CurrentCell.ColumnIndex + 1)
                BeginEdit()
            End If
    ...
    Private Sub grdpricelist_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles grdpricelist.Leave
        activecontrolname = ""
    End Sub

    Private Sub txtpricelistsearch_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtpricelistsearch.TextChanged
        grdpricelist.DataSource = SearchGrid(dtTable, txtpricelistsearch.Text, cmbpriceheader.SelectedIndex)
    End Sub

    Private Sub btnpricelistload_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnpricelistload.Click
        FillPriceList()
    End Sub
    ...
    Private Sub btnset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnset.Click
        ...
        Dim i As Integer
        For i = 0 To grdpricelist.RowCount - 1
            With grdpricelist
                If Val(txtdiscount.Text) > 0 Then
                    .Item("Discount", i).Value = Format(CDbl(txtdiscount.Text), numFormat)
                    .Item("tag", i).Value = "Y"
                End If
                'If .Item("tag", i).Value = "Y" Then

                'End If
                If Val(.Item("id", i).Value) > 0 Then
                    If Val(.Item("Discount", i).Value) > 0 Or Val(.Item("customerprice", i).Value) > 0 Then
                        _objcmnbLayer._saveDatawithOutParm("Update CustomerWisePriceDiscountTb set discount=" & CDbl(.Item("Discount", i).Value) & ",customerprice =" & CDbl(.Item("customerprice", i).Value) & " where id=" & Val(.Item("id", i).Value))
                    Else
                        _objcmnbLayer._saveDatawithOutParm("Delete from CustomerWisePriceDiscountTb where id=" & Val(.Item("id", i).Value))
                    End If
                Else
                    If Val(.Item("Discount", i).Value & "") > 0 Or Val(.Item("customerprice", i).Value & "") > 0 Then
                        If Val(.Item("Discount", i).Value & "") = 0 Then .Item("Discount", i).Value = 0
                        If Val(.Item("customerprice", i).Value & "") = 0 Then .Item("customerprice", i).Value = 0
                        _objcmnbLayer._saveDatawithOutParm("Insert into CustomerWisePriceDiscountTb(customerid,itemid,discount,customerprice) values(" & _
                                                       accid & "," & Val(.Item("itemid", i).Value) & "," & CDbl(.Item("Discount", i).Value) & "," & CDbl(.Item("customerprice", i).Value) & ")")
                    End If

                End If
            End With
            lblcount.Refresh()
            lblcount.Text = i + 1 & "/" & grdpricelist.RowCount
        Next
        lblcount.Text = "Updated " & lblcount.Text
        FillPriceList()
        MsgBox("Record Updated Successfully", MsgBoxStyle.Information)
        isModi = True
        AccNo = Val(txtRec0.Tag)
        'btnaddnew_Click(btnaddnew, New System.EventArgs)
        'If iscust Then
        '    ldCustomer()
        'Else
    ...
    Private Sub cmbBr_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cmbBr.KeyDown, cmbarea.KeyDown
        If e.KeyCode = Keys.Return Then
            SendKeys.Send("{TAB}")
        End If
    End Sub
    ...
    Private Sub grdpricelist_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdpricelist.CellContentClick

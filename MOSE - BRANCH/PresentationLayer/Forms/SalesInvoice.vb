...
#Region "Local Variables"
    ...
    Private iscustomerchange As Boolean
    'numeric text
...
#End Region
    ...
    Private Sub ldPostedInv()
        ...
        Dim itemquery As String = " SELECT ItmInvTrTb.*, [Item Code],Warrentyname,isSerialNo,fcode,FraCount," & _
                                          "isnull(itemCategory,'')itemCategory,collectionAC,vat,rgcess,rgcaccount,additionalcess,isnull(courseduration,0)courseduration FROM ItmInvTrTb LEFT JOIN InvItm ON InvItm.ItemId = ItmInvTrTb.ItemId " & _
        ...
        If Not IsDBNull(ItmInvCmnTb(0)("isfromexternal")) Then
            isfromexternal = ItmInvCmnTb(0)("isfromexternal")
        End If
        ...
        If Trim(ItmInvCmnTb(0)("cap2") & "") <> "" Then
            txtcap2.Text = Trim(ItmInvCmnTb(0)("cap2") & "")
        End If
        If Trim(ItmInvCmnTb(0)("cap3") & "") <> "" Then
            txtcap3.Text = Trim(ItmInvCmnTb(0)("cap3") & "")
        End If
        If Trim(ItmInvCmnTb(0)("cap4") & "") <> "" Then
            txtcap4.Text = Trim(ItmInvCmnTb(0)("cap4") & "")
        End If
        If Trim(ItmInvCmnTb(0)("cap5") & "") <> "" Then
            txtcap5.Text = Trim(ItmInvCmnTb(0)("cap5") & "")
        End If
        ...
        If isfromexternal Then
            MsgBox("You can't Post modifications in External Invoice", MsgBoxStyle.Exclamation)
        ...
        chgPost = False
        If userType Then
            btnRv.Visible = IIf(getRight(259, CurrentUser), False, True)
        Else
            btnRv.Visible = True
        End If
        ...
        With grdVoucher
            ...
            If sRs.Rows.Count > 0 Then
                For i = 0 To sRs.Rows.Count - 1
                    ...
                    If enableMembership Then
                        Dim expiry As Integer = Val(sRs(0)("courseduration") & "")
                        .Item(ConstUnitCount, i).Value = expiry
                    End If
...
ctn:
    ...
    Private Sub AddDetails(ByVal DR As DataTable)
        ...
        With grdVoucher
            ...
            If enableMembership Or enableSwimmingPool Or enableGYM Then
                Dim expiry As Integer = Val(DR(0)("courseduration") & "")
                .Item(ConstUnitCount, i).Value = expiry
                .Item(ConstWarrentyExpiry, i).Value = Format(DateAdd(DateInterval.Day, expiry - 1, DateValue(cldrdate.Value)), DtFormat)
            Else
            ...
            If CDbl(.Item(ConstUPrice, i).Value) = 0 Or chgItm Then
                ...
                ElseIf rdoother.Checked Then
                    .Item(ConstActualPrice, i).Value = DR(0)("Price" & rdoother.Tag)
                Else
                    .Item(ConstActualPrice, i).Value = 0
            ...
            End If
            If enableFloodCess And Val(lblgstn.Tag & "") = 0 And cessenddate >= DateValue(cldrdate.Value) Then
                'Total cessamt [REGULAR CESS+FLOOD CESS]
...
chkagain:
        ...
        TrId = Val(_objInv._saveCmn())
        If clinicvisitid > 0 Then
            _objcmnbLayer._saveDatawithOutParm("update ItmInvCmnTb set clinicvisitid=" & clinicvisitid & " where trid=" & TrId)
        End If
        ...
        Dim dt As DataTable
        'dt = _objcmnbLayer._fldDatatable("select trid from ItmInvCmnTb left join AccTrCmn on ItmInvCmnTb.TrId=AccTrCmn.LnkNo where isnull(lnkno,0)=0 and trid=" & TrId)
        dt = _objcmnbLayer._fldDatatable("select linkno from AccTrCmn where isnull(lnkno,0)=" & TrId)
        If dt.Rows.Count = 0 Then
            MsgBox("Error" & vbCrLf & "Amount Not updated in Accounts! Please modify the invoice", MsgBoxStyle.Critical)
            CheckNLoad(TrId)
            Exit Sub
        End If
                ...
                If enableMultipleDebitInInvoice And dtMultipleDebits.Rows.Count > 0 Then
                    If dtMultipleDebits.Rows.Count > 0 Then
                        ...
                        For j = 0 To dtMultipleDebits.Rows.Count - 1
                            ...
                            End If
                            'trinf=10 for RV
                            setAcctrDetValue(LinkNo, Val(dtMultipleDebits(j)("accid") & ""), dtMultipleDebits(j)("reference"), collectionRef, CDbl(dtMultipleDebits(j)("accAmt")) * FCRt, "", Strings.Left(txtJob.Text, 50), 10, 0, "", _
    ...
    Private Sub setCustomer(Optional ByVal accid As Long = 0, Optional ByVal skipCalculate As Boolean = False, Optional ByVal GSTN As String = "", Optional ByVal skipcustomerdiscount As Boolean = True, Optional ByVal varlinkno As Long = 0)
        ...
        If dt.Rows.Count > 0 Then
            ...
            ElseIf Val(dt(0)("pricegroup")) = 1 Then
                rdows.Checked = True
            ElseIf Val(dt(0)("pricegroup")) = 2 Then
                rdodealer.Checked = True
            Else
                rdoother.Checked = True
                rdoother.Tag = Val(dt(0)("pricegroup") & "") - 2
                rdoother.Text = "Other Price ( Price " & rdoother.Tag & " )"
            End If
            fetchPrice(True)
        Else
els:
        ...
        If rdoretail.Checked = True Then
            rdoother.Text = "Other Price"
        ElseIf rdows.Checked = True Then
            rdoother.Text = "Other Price"
        ElseIf rdodealer.Checked = True Then
            rdoother.Text = "Other Price"
...
u:
    ...
    Private Sub cldrdate_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles cldrdate.Validated
        ...
        If grdVoucher.RowCount = 0 Then Exit Sub
        With grdVoucher
            If enableSwimmingPool Or enableGYM Then
                Dim expiry As Integer
                For i = 0 To .RowCount - 1
                    expiry = Val(.Item(ConstUnitCount, i).Value)
                    .Item(ConstWarrentyExpiry, i).Value = Format(DateAdd(DateInterval.Day, expiry, DateValue(cldrdate.Value)), DtFormat)
                Next
            Else
                If Not grdVoucher.CurrentRow Is Nothing Then
    ...
    Private Sub formLoad()
        ...
        If userType Then
            ...
            If getRight(264, CurrentUser) Then
...
nw:
        ...
        If userType Then
            btnRv.Visible = IIf(getRight(259, CurrentUser), False, True)
            Label16.Visible = IIf(getRight(262, CurrentUser), False, True)
            lblbalance.Visible = IIf(getRight(262, CurrentUser), False, True)
        End If
        setPrice()
        chkb2b.Checked = enableB2BAsDefault
    End Sub
    ...
    Private Sub calculateTaxFromUnitPrice(ByVal i As Integer)
        If EnableGST = False And enableGCC = False And ShowTaxOnInventory = False Then Exit Sub
        If chgUprice And chkcal.Checked And chktaxInv.Checked And chktaxInv.Visible Then
            chgbyprg = True
            Dim actualprice As Double
            Dim ttax As Double
            With grdVoucher
    ...
    Private Sub btnprint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnprint.Click
        If enableDuplicateBill Then
            ...
            If frm.chkduplicate.Checked Then
                PrepareRpt("IS", True, 1)
            End If
            If frm.chktriplicate.Checked Then
                PrepareRpt("IS", True, 2)
            End If
        Else
            PrepareRpt("IS", True)
        End If
...
createNewItem:
    ...
    Private Sub txtCashCustomer_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtCashCustomer.Validated
        ...
        If iscustomerchange = False Then Exit Sub
        If vtype = "Credit" Then
            setCustomer(, , , False)
            txtDescr.Focus()
        Else
    ...
    'End Sub
    Private Sub fetchPrice(ByVal skipmessage As Boolean)
        'If Not enablefetchLastPrice Then Exit Sub
    ...
    End Sub
    Public Sub loadfromClinic(ByVal accid As Integer, ByVal refrence As String, Optional ByVal jobcode As String = "", _
                              Optional ByVal jobname As String = "", Optional ByVal withmedicine As Boolean = False, Optional ByVal visitid As Long = 0)
        ...
        Dim dtItms As DataTable
        If withmedicine Then
            Dim dt As DataTable
            Dim str As String = " select  [item code] from ClinicVisitMedicineTb " & _
            "left join ClinicVisitNoteTb on ClinicVisitNoteTb.cvn_visitnoteid=ClinicVisitMedicineTb.vnm_visitnoteid " & _
            "left join invitm on ClinicVisitMedicineTb.vnm_itemid=invitm.itemid  where cvn_visitid=" & visitid
            dt = _objcmnbLayer._fldDatatable(str)
            If dt.Rows.Count > 0 Then
                Dim i As Integer
                For i = 0 To dt.Rows.Count - 1
                    If Trim(dt(i)("item code") & "") <> "" Then
                        SrchText = dt(i)("item code")
                        dtItms = Nothing
                        dtItms = ItmValidation(3, SrchText, True, "IS", Val(txtSuppAlias.Tag))
                        If dtItms.Rows.Count > 0 Then
                            grdVoucher.Rows.Add(1)
    ...
    Public Sub returnFromJobCarWash(ByVal jobcode As String, ByVal jobid As Integer, ByVal plateno As String, ByVal cartype As String, ByVal customerid As Integer)
        ...
        End If
createNewItem:
        Dim dR As DataTable = ItmValidation(1, plateno, True, "IS", Val(txtSuppAlias.Tag))
        If dR.Rows.Count > 0 Then
            grdVoucher.Rows.Add(1)
            grdVoucher.CurrentCell = grdVoucher.Item(0, (grdVoucher.RowCount - 1))
            AddDetails(dR)
    ...
    End Sub
    Public Sub returnPoolMemberShip(ByVal jobid As Long)
        Dim strsql As String
        strsql = "SELECT jobcode,custcode from JobTb where JobTb.jobid=" & jobid
        Dim dtJob As DataTable = _objcmnbLayer._fldDatatable(strsql)
        chgbyprg = True
        Dim dt As DataTable
        dt = _objcmnbLayer._fldDatatable("SELECT [Voucher Name] FROM PreFixTb WHERE VrTypeNo = 4 AND Ctgry=3" & IIf(UsrBr = "", "", " AND BrId In ('', '" & UsrBr & "')") & " Order by ordNo")
    ...
    Private Sub RadioButton1_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rdoother.CheckedChanged
    ...
    Private Sub txtCashCustomer_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtCashCustomer.Validating
    ...
    Private Sub loadCustomerDetails(ByVal accid As Long)
        If Not fMList Is Nothing Then fMList.Close() : fMList = Nothing
        If txtCashCustomer.Text = "" And accid = 0 Then Exit Sub
        Dim dt As DataTable
        _objcmnbLayer = New clsCommon_BL
        Dim str As String
        If accid > 0 Then
        ...
        Else
            str = "SELECT accid,Alias,AccDescr,Address1,Address2,Address3,Address4,Phone,Fax,EMail,PT1,PT2,PT3,PT4," & _
                  "TrdLcno,TrdDate,ContactName,CountryCode from AccMast LEFT JOIN AccMastAddr ON AccMast.ACCID=AccMastAddr.accountno where AccDescr='" & txtCashCustomer.Text & "'"
        End If
        ...
        If dt.Rows.Count > 0 Then
            ...
            txtCashCustomer.Text = UCase(dt(0)("AccDescr"))
            txtCashCustomer.Tag = dt(0)("accid")

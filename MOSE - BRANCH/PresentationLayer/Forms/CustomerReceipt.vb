...
#Region "Local Variable Declerations"
    ...
    Private InstallmentRefrence As String
#End Region
    ...
    Private Sub grdVoucher_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdVoucher.CellClick
        ...
        If e.ColumnIndex = _vTag Then
            ...
            Else
                FillAmt(grdVoucher)
                If userType Then
                    If getRight(262, CurrentUser) = False Then
...
nxt:
    ...
    Private Sub grdVoucher_RowEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdVoucher.RowEnter
        ...
        loadinstallments(txtSuppName.Tag, grdVoucher.Item(_vRef, e.RowIndex).Value)
    End Sub
        ...
        If myctrl.Name = "txtSuppName" Then
            If chgCust Then
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    ldTrans()
                    If grdVoucher.RowCount > 0 Then
                        loadinstallments(Val(txtSuppName.Tag), grdVoucher.Item(_vRef, 0).Value)
                        loadInstamentId()
                    End If
                    Dim i As Integer
    ...
    End Sub
    Private Sub loadinstallments(ByVal customerid As Long, ByVal refrence As String)
        If InstallmentRefrence = refrence Then Exit Sub
        Dim dt As DataTable
        Dim str As String
        str = "select *,convert(varchar,installmentdate,103)+'  ||  '+convert(varchar,balance) instatext from (select installmentdate,amount-isnull(rvamt,0) balance,TrRefNo,cscode,id from InstallmentTb " & _
        "left join (select sum(DealAmt*-1)rvamt,insmntid from AccTrDet " & _
        "left join AccTrCmn ON AccTrDet.LinkNo=AccTrCmn.LinkNo where JVType='RV' and AccTrCmn.linkno<>" & loadedTrId & " group by insmntid)rv on InstallmentTb.id=rv.insmntid " & _
        "inner join ItmInvCmnTb on InstallmentTb.trid=ItmInvCmnTb.TrId)tr where balance>0 and cscode=" & customerid & " and TrRefNo='" & refrence & "' order by installmentdate"
        ...
        If dt.Rows.Count > 0 Then
            ...
            For i = 0 To dt.Rows.Count - 1
                If Val(cmbinstallment.Tag) = dt(i)("id") Then
                    selected = i
                End If
                cmbinstallment.Items.Add(dt(i)("instatext"))
            Next
        End If
        If cmbinstallment.Items.Count > 0 Then
            cmbinstallment.SelectedIndex = selected
        End If
    ...
    Private Sub btnupdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnupdate.Click
        ...
        Else
            ...
            End If
        End If
        If enableRVApproval Then
            If btnapprove.Text = "Undo Approve" Then
                MsgBox("Already approved RV cannot be updated", MsgBoxStyle.Exclamation)
                Exit Sub
            End If
        End If
        assaignTotal()
    ...
    Private Sub Verify()
        ...
        End If
        If enableInstallmentInRV And enableinstallmentalert Then
            Dim i As Integer
                ...
                With grdVoucher
                    If .Item(_vTag, i).Value = "Y" Then
                        ...
                        If CDbl(.Item(_vAssign, i).Value) <> CDbl(.Item(_installment, i).Value) Then
                            If MsgBox("Assigned Amount does not match with installment amount! " & vbCrLf & "Do you want to continue?", MsgBoxStyle.Question + MsgBoxStyle.YesNo + MsgBoxStyle.DefaultButton2) = MsgBoxResult.No Then Exit Sub
                        End If
                    End If
                End With
            Next
...
chkagain:
        ...
        With grdVoucher
            For i = 0 To .RowCount - 1
                ...
                If enableschoolmanagement Then
                    entryref = Trim(.Item(_vEntryRef, i).Value & "")
                Else
                    entryref = entryref & " / " & txtDescription.Text
                End If
                setAcctrDetValue(LinkNo, i, grdVoucher, 0, paymentaccount, Val(.Item(_SetoffCount, i).Value & ""), entryref)
...
nxt1:
        ...
        If userType And enableRVApproval Then
            setapproval = 1
        End If
        _objTr.SaveAccTrWithDt(dtAccTb, 0, setapproval, Val(cmbinstallment.Tag))
        numPrintVchr.Text = numVchrNo.Text
...
nxt:
    ...
    Private Sub setAcctrDetValue(ByVal lnkNo As Long, ByVal _row As Integer, ByVal grd As DataGridView, _
        ...
        dtrow("setoffCount") = setoffCount
        If enableChooseInstallmentinRV And trtype = 0 Then
            dtrow("insmntid") = Val(cmbinstallment.Tag)
        End If
        If trtype = 0 Then
            dtrow("UnqNo") = 0
        ...
        Else
            dtrow("UnqNo") = 0  'Val(grd.Item(ConstUnq, _row).Value)
...
nxt:
    ...
    Private Sub LoadReport(ByVal FileName As String, ByVal RptCaption As String, Optional ByVal ToPrint As Boolean = False)
        ...
        If TabControl1.SelectedIndex = 0 Then
            ...
            If STRptType = "ST1" Then
                dt = _objcmnbLayer._fldDatatable("select accid from accmast where accid=" & Val(txtSuppName.Tag))
                If dt.Rows.Count > 0 Then
                    accid = dt(0)("accid")
    ...
    Private Sub ldRec()
        ...
        If dtAccTrCmn.Rows.Count > 0 Then
            ...
            cmbinstallment.Tag = Val(dtAccTrCmn(0)("courseInstallmentid") & "")
            ...
            For i = 0 To grdVoucher.RowCount - 1
                ...
                End If
            Next
            loadinstallments(Val(txtSuppName.Tag), grdVoucher.Item(_vRef, refRow).Value)
            activecontrolname = ""
            If chqStatus Then
                btndelete.Enabled = False
                btnupdate.Enabled = False
    ...
    Private Sub CustomerReceipt_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If txtSuppName.Text <> "" Or editlinkno > 0 Then
            Timer2.Enabled = True
        End If
        Label17.Visible = enableChooseInstallmentinRV
        cmbinstallment.Visible = enableChooseInstallmentinRV
    End Sub
...
nxt:
    ...
    Private Sub SetGridTr()
        With grdVoucher
            ...
            .Columns(_SetoffCount).HeaderText = "Setoff Count"
            .Columns(_SetoffCount).Width = 100
            ...
            .Columns(_installment).Visible = enableInstallmentInRV
            If userType Then
                .Columns(_SetoffCount).Visible = IIf(getRight(255, CurrentUser), False, True)
                .Columns(_vSetOffAmt).Visible = IIf(getRight(258, CurrentUser), False, True)
                .Columns(_vBalance).Visible = IIf(getRight(262, CurrentUser), False, True)
            End If
    ...
    Private Sub cmbinstallment_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbinstallment.SelectedIndexChanged
        If chgbyprg Then Exit Sub
        loadInstamentId()
    End Sub
    Private Sub loadInstamentId()
        Dim dt As DataTable
        Dim str As String
        Dim customerid As Long = txtSuppName.Tag
        InstallmentRefrence = grdVoucher.Item(_vRef, grdVoucher.CurrentRow.Index).Value
        str = "select * from (select *,convert(varchar,installmentdate,103)+'  ||  '+convert(varchar,balance) instatext from (select installmentdate,amount-isnull(rvamt,0) balance,TrRefNo,id,cscode from InstallmentTb " & _
        "left join (select sum(DealAmt*-1)rvamt,insmntid from AccTrDet " & _
        ...
        "inner join ItmInvCmnTb on InstallmentTb.trid=ItmInvCmnTb.TrId)tr where balance>0 and cscode=" & customerid & " and TrRefNo='" & InstallmentRefrence & "')tr where instatext='" & cmbinstallment.Text & "'"
        dt = _objcmnbLayer._fldDatatable(str)
        If dt.Rows.Count > 0 Then
            cmbinstallment.Tag = dt(0)("id")
            If grdpayment.RowCount > 0 Then
                grdpayment.Item(ConstPAmount, 0).Value = Format(dt(0)("balance"), numFormat)
            End If
        End If
        Dim i As Integer
        For i = 0 To grdVoucher.RowCount - 1
            grdVoucher.Item(_vAssign, i).Value = 0
            grdVoucher.Item(_vTag, i).Value = ""
        Next
        assaignTotal()
    End Sub

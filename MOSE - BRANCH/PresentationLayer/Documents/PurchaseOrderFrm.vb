...
#End Region
    ...
    Public Sub CheckNLoad(Optional ByVal FromTrId As Long = 0)
        ...
        Else
            InvList = _objcmnbLayer._fldDatatable("SELECT Docid FROM DocCmnTb  WHERE Doctype = 'PO' AND DNO = " & Val(numVchrNo.Text) & _
    ...
    Private Sub ldPostedInv()
        ...
        strquery = "SELECT DocCmnTb.*,jobname,fname,AccDescr,alias,cartype,platenumber FROM DocCmnTb " & _
                                                  ...
                                                  "WHERE DocId = " & loadedTrId & " AND DocType = 'PO'"
        strquery = strquery & " SELECT DocTranTb.*, [Item Code],vat,foundImport,rgcess,additionalcess FROM DocTranTb " & _
        ...
        Dim dtset As DataSet = _objcmnbLayer._ldDataset(strquery, False)
        chgbyprg = True
...
Quit:
    ...
    Private Sub NextNumber()
        ...
        getVrsDet(0, "PO", vrPrefix, vrVoucherNo, vrAccountNo1, vrAccountNo2)
        If Not isModi Then
        ...
        With grdVoucher
            Select Case ColIndex
                Case ConstBarcode, ConstItemCode
                    ...
                    dtItms = ItmValidation(3, SrchText, True, "IP", Val(txtSuppAlias.Tag))
    ...
    Private Sub AddDetails(ByVal DR As DataTable)
        ...
        With grdVoucher
            ...
            If CDbl(.Item(ConstUPrice, i).Value) = 0 Or chgItm Then
                If Val(DR(0)("lastPrice") & "") = 0 Then
                    .Item(ConstActualPrice, i).Value = DR(0)("LastPurchCost")
                Else
...
nxt:
    ...
    Private Sub btnModify_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnModify.Click
        If btnModify.Text = "&Modify" Then
            ...
            If userType Then
                btnupdate.Tag = IIf(getRight(220, CurrentUser), 1, 0)
                btndelete.Tag = IIf(getRight(221, CurrentUser), 1, 0)
            Else
                btnupdate.Tag = 1
                btndelete.Tag = 1
            End If
    ...
    Private Sub saveTrans()
        ...
        If Not isModi Then
chkagain:
            If Not CheckNoExists(txtprefix.Text, Val(numVchrNo.Text), "PO", "Document") Then
        ...
        If Not isModi Then
            ...
            txtprefix.Text = SetNextVrNo(numVchrNo, 0, "PO", "DocType = 'PO' AND DNO = ", False, False, True, 0, txtprefix.Text)
        End If
        ChgId = False
        chgPost = False
        AddNewClick()
        MsgBox("Purchase Order is succesfully posted with DOC. # " & numPrintVchr.Text & ".", MsgBoxStyle.Information)
        numVchrNo.Tag = ""
    End Sub
...
ChkAgain:
        dtTable = _objcmnbLayer._fldDatatable("SELECT * FROM DocCmnTb WHERE DocType = 'PO' AND DNo = " & Val(numVchrNo.Text))
...
skip:
    ...
    Private Sub fSlctDoc_selectTr(ByVal trid As Long, ByVal TrType As String) Handles fSlctDoc.selectTr
        If Val(btnimportDoc.Tag) > 0 Then
            PasteFrom(trid)
        Else
            CheckNLoad(trid)
        End If
    ...
    Private Sub SalesOrderFrm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            Else
                AddNewClick()
                If userType Then
                    btnupdate.Tag = IIf(getRight(219, CurrentUser), 1, 0)
                Else
                    btnupdate.Tag = 1
                End If
                btndelete.Text = "Clear"

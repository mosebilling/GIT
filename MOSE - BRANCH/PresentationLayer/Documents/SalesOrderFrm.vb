...
#End Region
    ...
    Private Sub ldPostedInv()
        ...
        cmbsalesman.Text = Trim("" & ItmInvCmnTb(0)("SlManId"))
        If Not IsDBNull(ItmInvCmnTb(0)("DueDt")) Then
            dtpduedate.Value = DateValue(ItmInvCmnTb(0)("DueDt"))
        End If
...
nxt:
    ...
    Private Sub ClearControls()
        ...
        dtpduedate.Value = Format(Date.Now, DtFormat)
        chgNumByPgm = False
        'add_Click()
        'grdVoucher.Select()
...
ter:
    ...
    Private Sub setCmnValue()
        ...
        With _objDoc
            ...
            .DueDt = DateValue(dtpduedate.Value)
...
skip:
    ...
    Private Sub setCustomer(Optional ByVal accid As Long = 0)
        ...
        dt = _objcmnbLayer._fldDatatable("SELECT AccMast.accid,Alias,AccDescr,Address1,Address2,Address3,Address4," & _
                                        "Phone,Fax,EMail,PT1,PT2,PT3,PT4,TrdLcno,TrdDate,CreditLimit,DueDays,SlsmanId,CurrencyCode,CountryCode,GSTIN,Remarks,ISNULL(pricegroup,0)pricegroup " & _
...
u:
    ...
    Private Function ImportDOs(ByVal Docids As String, Optional ByVal trasferAllItems As Boolean = False) As Boolean
        ...
        If trasferAllItems Then
            sRs = _objcmnbLayer._fldDatatable("SELECT * from (Select DocTranTb.*, [Item Code],isSerialNo,FraCount,isnull(itemCategory,'')itemCategory," & _
                                              ...
                                              "Discount,rndoff,cscode,accdescr,alias FROM DocTranTb LEFT JOIN DocCmnTb ON DocCmnTb.DocId=DocTranTb.DocId " & _
                                              "LEFT JOIN AccMast on Accmast.accid=DocCmnTb.cscode " & _
                                              "LEFT JOIN InvItm ON InvItm.ItemId = DocTranTb.ItemId " & _
            ...
            If sRs.Rows.Count > 0 Then
                For i = 0 To sRs.Rows.Count - 1
                    If i = 0 Then
                        ...
                        End If
                        txtSuppName.Text = sRs(i)("accdescr")
                        txtSuppAlias.Tag = sRs(i)("cscode")
                        txtSuppAlias.Text = sRs(i)("alias")
                    End If
    ...
    Private Sub rdoretail_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles rdoretail.Click, rdodealer.Click, rdows.Click
        fetchPrice(False)
    End Sub
    Private Sub getInvNos()
        If loadedTrId = 0 Then Exit Sub
        Dim dt As DataTable
        Dim invnos As String = ""
        dt = _objcmnbLayer._fldDatatable("select prefix,invno from ItmInvCmnTb left join ItmInvTrTb on ItmInvTrTb.trid=ItmInvCmnTb.trid where impDocid=" & loadedTrId)
        If dt.Rows.Count > 0 Then
            invnos = IIf(invnos = "", "", ",") & Trim(dt(0)("prefix") & "") & IIf(Trim(dt(0)("prefix") & "") = "", "", "/") & dt(0)("invno")
        End If
        txtinvno.Text = invnos
    End Sub

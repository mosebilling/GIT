...
#Region "Public Variables"
    ...
    Public Event returnqtidetails(ByVal trid As Long)
#End Region
...
#End Region
    ...
    Public Sub CheckNLoad(Optional ByVal FromTrId As Long = 0)
        ...
        Else
            InvList = _objcmnbLayer._fldDatatable("SELECT Docid FROM DocCmnTb  WHERE Doctype = 'QTI' AND DNO = " & Val(numVchrNo.Text) & _
    ...
    Private Sub ldPostedInv()
        ...
        strqry = "SELECT DocCmnTb.*,jobname,fname,AccDescr,alias,cartype,platenumber FROM DocCmnTb " & _
        ...
        strqry = strqry & " SELECT DocTranTb.*, [Item Code],vat,foundImport,rgcess,additionalcess FROM DocTranTb " & _
        ...
        txtcustAddress.Text = Trim("" & ItmInvCmnTb(0)("custAddress"))
        cmbfc.Text = Trim("" & ItmInvCmnTb(0)("FC"))
        If Val(ItmInvCmnTb(0)("FCRate") & "") > 0 Then
            txtfcrt.Text = Format(CDbl(ItmInvCmnTb(0)("FCRate")), lnumFormat)
        Else
            txtfcrt.Text = Format(1, lnumFormat)
        End If
        If ItmInvCmnTb(0)("Cancelled") = True Then
            btncancel.Text = "Undo Cancel"
        Else
        ...
        With grdVoucher
            ...
            If sRs.Rows.Count > 0 Then
                For i = 0 To sRs.Rows.Count - 1
                    ...
                    grdVoucher.Item(ConstUPrice, i).Value = Format(grdVoucher.Item(ConstActualPrice, i).Value, "#,##0" & IIf(NDec = 0, "", "." & Strings.StrDup(NDec, "0"))) ', "#,##" & NumFormat)
                    ...
                    If Val(sRs(i)("lineCost") & "") = 0 Then sRs(i)("lineCost") = 0
                    grdVoucher.Item(ConstBatchCost, i).Value = sRs(i)("lineCost") / FCRt
...
Quit:
        ...
        With grdVoucher
            ...
            If sRs.Rows.Count > 0 Then
                For i = 0 To sRs.Rows.Count - 1
                    ...
                    If Val(sRs(i)("ItemDiscount") & "") = 0 Then sRs(i)("ItemDiscount") = 0
                    grdVoucher.Item(ConstDisAmt, i).Value = Format(sRs(i)("ItemDiscount") * UPerPack / FCRt, lnumFormat)
                    grdVoucher.Item(ConstImpDocId, i).Value = 0
                    grdVoucher.Item(ConstImpLnId, i).Value = 0
    ...
    Private Sub txtReference_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtSuppName.KeyDown, txtSuppAlias.KeyDown, txtReference.KeyDown, _
                                                                                                                    txtjobname.KeyDown, txtJob.KeyDown, txtAttn.KeyDown, txtsubject.KeyDown
        lstKey = e.KeyCode
    ...
    Private Sub txtLocation_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtJob.TextChanged, txtSuppName.TextChanged, _
                                                                                                            txtSuppAlias.TextChanged, txtsubject.TextChanged
    ...
    Private Sub ShowFmlist(ByVal myCtrl As Control)
        ...
        Select Case _srchTxtId
            ...
            Case 4   'subject
                ...
                fMList.SearchIndexDescr = 0
                fMList_doFocus()
                fMList.Search(txtsubject.Text)
                fMList.AssignList(txtsubject, lstKey, chgbyprg)
        End Select
        _srchOnce = True
        chgbyprg = False
    End Sub
    ...
    Private Sub SetGridHead()
        ...
        With grdVoucher
            ...
            .Columns(constItmTot).Visible = enableTaxinDocument And (enableGCC = True Or EnableGST = True)
            .Columns(ConstTaxP).Visible = enableTaxinDocument And (enableGCC = True Or EnableGST = True)
            .Columns(ConstTaxAmt).Visible = enableTaxinDocument And (enableGCC = True Or EnableGST = True)
            .Columns(ConstcessAmt).Visible = enableTaxinDocument And (enableGCC = True Or EnableGST = True)
            .Columns(ConstBatchCost).Visible = True
            .Columns(ConstLineProfit).Visible = True
        ...
        With grdVoucher
            For i = 0 To .Rows.Count - 1
                ...
                Else
                    ...
                    End If
                    If enableFloodCess And Val(lblgstn.Tag & "") = 0 And cessdate <= DateValue(cldrdate.Value) And cessenddate >= DateValue(cldrdate.Value) Then
...
nxt:
            ...
            lblprofit.Text = Format(ttlprofit - CDbl(numDisc.Text), numFormat)
            chgAmt = False
            chgbyprg = False
        End With
    End Sub
        ...
        Try
            With grdVoucher
                ...
                If Val(grdVoucher.Item(0, grdVoucher.CurrentCell.RowIndex).Value) = 0 And e.ColumnIndex <> 3 Then
                    grdVoucher.CurrentCell.ReadOnly = True
                ElseIf e.ColumnIndex <> ConstSlNo And e.ColumnIndex <> constItmTot And _
                e.ColumnIndex <> ConstUnit And e.ColumnIndex <> ConstLTotal And _
                e.ColumnIndex <> ConstBarcode And e.ColumnIndex <> ConstTaxAmt And _
                e.ColumnIndex <> ConstStartReading And e.ColumnIndex <> ConstMRP And e.ColumnIndex <> ConstLineProfit Then
    ...
    Private Sub AddDetails(ByVal DR As DataTable)
        ...
        With grdVoucher
            ...
            If Val(DR(0)("CostAvg") & "") > 0 Then
                .Item(ConstBatchCost, i).Value = Format(CDbl(DR(0)("CostAvg")), numFormat)
            Else
                .Item(ConstBatchCost, i).Value = Format(0, numFormat)
            End If
            ...
            .Item(ConstLineProfit, i).Value = Format(CDbl(.Item(ConstActualPrice, i).Value) - CDbl(.Item(ConstBatchCost, i).Value), lnumFormat)
        ...
        With grdVoucher
            ...
            Select Case e.ColumnIndex
                Case ConstItemCode, ConstBarcode
                    chgItm = True
                Case ConstQty, ConstTaxP, ConstDisAmt, ConstDisP, ConstWoodDiscQty, ConstWoodQty, ConstBatchCost
                    chgAmt = True
                Case ConstLTotal
            ...
            With grdVoucher
                If .Columns(i).Visible = True Then
                    ...
                    ElseIf Val(.Item(ConstDonotAllowsaveItem, .CurrentCell.RowIndex).Value) = 1 And i <> 3 Then
                        .Columns(i).ReadOnly = True
                    Else
                        If i <> ConstSlNo And i <> constItmTot And i <> ConstUnit And _
                        i <> ConstLTotal And i <> ConstBarcode And i <> ConstTaxAmt And _
                        i <> ConstStartReading And i <> ConstcessAmt And i <> ConstLineProfit Then
                            If i = ConstDescr And disableEditProdectDescription Then
                                .Columns(i).ReadOnly = True
                            Else
...
cntu:
                diableColums()
                'MsgBox((grdVoucher.Rows(0).Cells(ConstLineProfit)).ReadOnly)
...
nxt:
    ...
    Private Sub ClearControls()
        ...
        chgbyprg = False
        btncancel.Text = "Cancel"
        If Not fImportdcos Is Nothing Then fImportdcos.Close() : fImportdcos = Nothing
        On Error GoTo 0
    End Sub
    ...
    Private Sub btnModify_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnModify.Click
        If btnModify.Text = "&Modify" Then
            ...
            If userType Then
                btnupdate.Tag = IIf(getRight(205, CurrentUser), 1, 0)
                btndelete.Tag = IIf(getRight(206, CurrentUser), 1, 0)
            Else
                btnupdate.Tag = 1
                btndelete.Tag = 1
            End If
    ...
    Private Sub saveTrans()
        ...
        If Not isModi Then
chkagain:
            If Not CheckNoExists(txtprefix.Text, Val(numVchrNo.Text), "QTI", "Document") Then
        ...
        If Not isModi Then
            ...
            numVchrNo.Tag = numVchrNo.Text
            txtprefix.Text = SetNextVrNo(numVchrNo, 0, "QTI", "DocType = 'QTI' AND DNO = ", False, False, True, 0, txtprefix.Text)
        End If
        ChgId = False
        chgPost = False
        AddNewClick()
        MsgBox("Customer Quotation is succesfully posted with DOC. # " & numPrintVchr.Text & ".", MsgBoxStyle.Information)
        numVchrNo.Tag = ""
    End Sub
    Private Sub setDetValueTr()
        ...
        With grdVoucher
            For i = 0 To .Rows.Count - 1
                ...
                dtrow("CostPUnit") = CDbl(.Item(ConstActualPrice, i).Value) * FCRt * PPerU 'CDbl(.TextMatrix(i, 10)) / PPerU
                slno = slno + 1
                dtrow("SlNo") = slno
                dtrow("PFraction") = PPerU
                dtrow("impDocid") = Val(.Item(ConstImpDocId, i).Value & "")
                dtrow("ImpDocLnNo") = Val(.Item(ConstImpLnId, i).Value & "")
                ...
                dtrow("lineCost") = CDbl(.Item(ConstBatchCost, i).Value)
...
skip:
    ...
    Private Sub setDetValue(ByVal Invid As Long, ByVal PPerU As Single, ByVal i As Integer)
        With grdVoucher
            ...
            _objDoc.lineCost = (CDbl(grdVoucher.Item(ConstBatchCost, i).Value) * FCRt)
...
ter:
    ...
    Private Sub setCmnValue()
        _objDoc = New clsDocCmn
        With _objDoc
            .DocType = IIf(isGarrage, "JQTI", "QTI")
            ...
            .FromLoc = ""
            .DocDefLoc = ""
            .Discount = Format(CDbl(numDisc.Text), numFormat)
            If Val(txtfcrt.Text) = 0 Then txtfcrt.Text = 0
            .FCRate = CDbl(txtfcrt.Text)
            .FC = cmbfc.Text
    ...
    Private Sub fSlctDoc_selectTr(ByVal trid As Long, ByVal TrType As String) Handles fSlctDoc.selectTr
        If Val(btnpastefrom.Tag) > 0 Then
            PasteFrom(trid)
        Else
            CheckNLoad(trid)
        End If
    ...
    Private Sub txtDescr_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtDescr.TextChanged, txtReference.TextChanged, txtAttn.TextChanged
        If chgbyprg Then Exit Sub
        chgPost = True
        'btnUpdate.Enabled = Val(btnUpdate.Tag) > 0
    End Sub
    ...
    Private Sub DeleteClick()
        ...
        If MsgBox("You are going to REMOVE the Sales Invoice # " & numVchrNo.Text & Chr(13) & "Are you sure ?", vbOKCancel + vbQuestion + vbDefaultButton2) = vbOK Then
            _objcmnbLayer._saveDatawithOutParm("delete from DocCmnTb where DocId=" & loadedTrId & " delete from DocTranTb where DocId=" & loadedTrId)
...
u:
    ...
    Private Sub grdVoucher_CellEndEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdVoucher.CellEndEdit
        ...
        chgbyprg = True
        If col = ConstQty Or col = ConstUPrice Or col = ConstDisAmt Or col = ConstLTotal Or _
        col = ConstTaxP Or col = ConstDisP Or col = ConstEndReading Or _
        col = ConstWoodDiscQty Or col = ConstWoodQty Or col = ConstBatchCost Then
    ...
    Private Sub FormLoad()
        ...
        loadDocumentMasters("QTI", quoteTerms)
        AddDttoCombo(cmbsalesman, dtsalesman, True, False)
        AddDttoCombo(cmbfc, dtcurrentyTb, True, False)
    End Sub
    Private Sub CustomerQuotation_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            FormLoad()
            ...
            Else
                AddNewClick()
                If userType Then
                    btnupdate.Tag = IIf(getRight(204, CurrentUser), 1, 0)
                Else
                    btnupdate.Tag = 1
                End If
                btndelete.Text = "Clear"
            ...
            chgPost = False
            Timer1.Enabled = True
            chkcal.Checked = calcluatetaxFrompricedoc
    ...
    Private Sub chkcal_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles chkcal.Click
        _objcmnbLayer._saveDatawithOutParm("Update CompanyTb set calcluatetaxFrompricedoc =" & IIf(chkcal.Checked, 1, 0))
        calcluatetaxFrompricedoc = chkcal.Checked
    End Sub

    Private Sub btnpastefrom_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnpastefrom.Click
        btnpastefrom.Tag = 1
        fSlctDoc = New SelectInvTr
        With fSlctDoc
            .strType = "QTI"
    ...
    Private Sub btncancel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btncancel.Click
        ...
        If btncancel.Text = "Cancel" Then
            If MsgBox("Do you want to cancel the document?", MsgBoxStyle.YesNo + MsgBoxStyle.Question) = MsgBoxResult.No Then Exit Sub
            _objcmnbLayer._saveDatawithOutParm("Update DocCmnTb set Cancelled=1 where docid=" & loadedTrId)
            btncancel.Text = "Undo Cancel"
        Else
            If MsgBox("Do you want to Undo cancel the document?", MsgBoxStyle.YesNo + MsgBoxStyle.Question) = MsgBoxResult.No Then Exit Sub
            _objcmnbLayer._saveDatawithOutParm("Update DocCmnTb set Cancelled=0 where docid=" & loadedTrId)
            btncancel.Text = "Cancel"
        End If

...
Public Class FruitSalesReturnInvoice
...
#End Region
    ...
    Private Sub ldPostedInv()
        ...
        loadCarriers(IIf(creditCustomerACC > 0, creditCustomerACC, Val(txtSuppAlias.Tag)), loadedTrId, True)
...
Quit:
    ...
    Private Sub PasteFrom(ByVal Trid As Long)
        ...
        If dtInvCmn.Rows.Count > 0 Then
            ...
            If Val(dtInvCmn(0)("ccustid") & "") = 0 Then
                creditCustomerACC = 0 ' Val(ItmInvCmnTb(0)("CSCode" & ""))
            Else
                creditCustomerACC = Val(dtInvCmn(0)("ccustid" & ""))
                'txtCashCustomer.Enabled = False
            End If
            ...
            If IsReturn Then
                ...
                End If
                loadCarriers(IIf(creditCustomerACC > 0, creditCustomerACC, dtInvCmn(0)("CSCode")), 0, False)
                txtSuppName.Enabled = False
        ...
        Try
            ...
            With cmbVoucherTp
                If .Items.Count > 0 Then
                    ...
                    getVrsDet(Val(cmbVoucherTp.Tag), "SR", vrPrefix, vrVoucherNo, vrAccountNo1, vrAccountNo2)
                Else
    ...
    Private Sub SetGridHeadCarrier()
        ...
        With grdcarrier
            ...
            .Columns(constCarrierCurrentVoucher).Visible = False
        ...
        With grdVoucher
            ...
            Select Case e.ColumnIndex
                Case ConstItemCode, ConstBarcode
                    chgItm = True
                Case ConstQty, ConstUPrice, ConstTaxP, ConstDisP, ConstDisAmt, ConstWoodQty
...
ter:
    ...
    Private Sub txtSuppAlias_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtSuppAlias.Validated, txtSuppName.Validated
        ...
        If dt.Rows.Count > 0 Then
            ...
            '    End If
            'End If
            loadCarriers(Val(txtSuppAlias.Tag), 0, False)
        Else
            txtSuppAlias.Text = ""
            txtSuppName.Text = ""
            lbladd1.Text = ""
...
u:
    ...
    Private Sub updateStockTransaction(ByVal trid As Long, ByVal LinkNo As Long)
        ...
        End If
        If costAmt <> 0 Then
            Dim entryref As String = "SALES RETURN : " & txtSuppName.Text & " # " & Trim(txtprefix.Text) & numVchrNo.Text
            'debit entry [stock in hand]
    ...
    End Sub
    Private Sub loadCarriers(ByVal customerid As Long, ByVal trid As Long, ByVal isfromInvoice As Boolean)
        Dim dt As DataTable
...
n:
            str = "select carriername,isnull(obQtyIn,0)obQtyIn,isnull(obQtyOut,0)obQtyOut,isnull(QtyIn,0)QtyIn,isnull(QtyOut,0)QtyOut,CarrierTb.carrierid,isnull(carriertrid,0)carriertrid,0 trOb from CarrierTb " & _
              "left join ( SELECT QtyIn,QtyOut,carrierid,carriertrid FROM CarrierTrTb where trid=" & trid & " and isnull(trid,0)>0)CarrierTrTb on CarrierTrTb.carrierid=CarrierTb.carrierid " & _
        ...
        ElseIf trid > 0 Then
            If isfromInvoice Then
                dt = _objcmnbLayer._fldDatatable("select customerid from CarrierTrTb where trid=" & trid & " group by customerid")
                If dt.Rows.Count > 0 Then
            ...
            str = "select carriername,isnull(obQtyIn,0)obQtyIn,isnull(obQtyOut,0)obQtyOut,isnull(QtyIn,0)QtyIn,isnull(QtyOut,0)QtyOut,CarrierTb.carrierid,isnull(carriertrid,0)carriertrid,isnull(trOb,0)trOb from CarrierTb " & _
              "left join ( SELECT trOb,QtyIn,QtyOut,carrierid,carriertrid FROM CarrierTrTb where trid=" & trid & " and isnull(trid,0)>0)CarrierTrTb on CarrierTrTb.carrierid=CarrierTb.carrierid " & _
              "left join(select sum(QtyIn)obQtyIn,sum(QtyOut)obQtyOut,carrierid OBcarrierid from  CarrierTrTb where customerid=" & customerid & _
              " AND trid<" & trid & " group by carrierid)OBCarrierTrTb on OBCarrierTrTb.OBcarrierid=CarrierTb.carrierid"
        End If
        ...
        For i = 0 To dt.Rows.Count - 1
            With grdcarrier
                ...
                .Item(constCarrierCurrentVoucher, i).Value = 0
                ...
                .Item(constcarrierbal, i).Value = Val(.Item(constoldbalance, i).Value) - Val(.Item(constcarrierReturn, i).Value)
                chgbyprg = False
            End With
        Next
        calculateCarrier()
        ...
        With grdcarrier
            For i = 0 To .RowCount - 1
                ...
                For r = 0 To grdVoucher.RowCount - 1
                    ...
                    End If
                Next
                .Item(constcarrierReturn, i).Value = cqty
    ...
    Private Sub saveCarrier(ByVal trid As Long)
        Dim i As Integer
        Dim qry As String
        Dim trRemarks As String = "TRANSACTION FROM SALES RETURN"
        With grdcarrier
            For i = 0 To .RowCount - 1

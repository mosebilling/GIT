...
Public Class FruitSalesInvoice
...
#Region "Const Variables"
    ...
    Private Const constCarrierCurrentVoucher = 1
    Private Const constoldbalance = 2
...
#End Region
    ...
    Private Sub ldPostedInv()
        ...
        If Val(ItmInvCmnTb(0)("ccustid") & "") = 0 Then
            creditCustomerACC = 0 ' Val(ItmInvCmnTb(0)("CSCode" & ""))
        Else
        ...
        setCustomer(ItmInvCmnTb(0)("CSCode"), True, Trim(ItmInvCmnTb(0)("GSTN") & ""), , varlinkno, True)
    ...
    Private Sub NextNumber()
        ...
        Try
            ...
            If dtTable.Rows.Count > 0 Then
                ...
                If txtCashCustomer.Text = "" And creditCustomerACC = 0 Then txtCashCustomer.Text = txtSuppName.Text
                chgbyprg = False
            Else
                txtSuppName.Text = ""
                txtSuppName.Text = ""
...
getcust:
    ...
    Private Sub SetGridHead()
        ...
        With grdVoucher
            ...
            .Columns(ConstWoodQty).Visible = True
            .Columns(ConstWoodQty).HeaderText = "C. Qty"
            .Columns(ConstWoodQty).DefaultCellStyle.Format = "N0"
            .Columns(ConstWoodQty).DefaultCellStyle.BackColor = Color.LightPink
            If enableFuleBankInvoice Then
    ...
    Private Sub SetGridHeadCarrier()
        ...
        With grdcarrier
            ...
            .Columns(constoldbalance).HeaderText = "Old Balance"
            ...
            .Columns(constCarrierCurrentVoucher).HeaderText = "Issued"
            ...
            .Columns(constcarrierbal).DefaultCellStyle.BackColor = Color.GreenYellow
...
ctn:
        ...
        With grdVoucher
            ...
            If dtQty.Rows.Count > 0 Then
                ...
                If dtQty(0)("Cost") > unitprice And EnableAlertBelowcost And checkcost Then
                    ...
                    End If
                Else
                    .Item(ConstDonotAllowsaveItem, i).Value = IIf(Val(.Item(ConstDonotAllowsaveItem, i).Value) = 2, 2, itemexceeds)
                End If
            End If
        End With
    End Sub
    Private Sub AddDetails(ByVal DR As DataTable)
        ...
        With grdVoucher
            ...
            .Item(Constsman, i).Value = Trim(DR(0)("carrier") & "")
...
ter:
    ...
    Private Sub txtSuppAlias_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtSuppAlias.Validated, txtSuppName.Validated
        ...
        setCustomer()
    End Sub
    Private Sub setCustomer(Optional ByVal accid As Long = 0, Optional ByVal skipCalculate As Boolean = False, _
                            Optional ByVal GSTN As String = "", Optional ByVal skipcustomerdiscount As Boolean = True, _
                            Optional ByVal varlinkno As Long = 0, Optional ByVal isfromInvoice As Boolean = False)
        Dim dt As DataTable
        ...
        If dt.Rows.Count > 0 Then
            ...
            If Not isNotCustomerAccount Then
                ...
                If EnableFruitsSales Then
                    If creditCustomerACC > 0 And isfromInvoice Then
                        loadCarriers(creditCustomerACC, isfromInvoice)
                    Else
                        loadCarriers(Val(txtSuppAlias.Tag), isfromInvoice)
                    End If
                End If
            Else
...
els:
    ...
    End Sub
    Private Sub loadCarriers(ByVal customerid As Long, ByVal isfromInvoice As Boolean)
        Dim dt As DataTable
        ...
        If loadedTrId = 0 Then
n:
            str = "select carriername,isnull(obQtyIn,0)obQtyIn,isnull(obQtyOut,0)obQtyOut,isnull(QtyIn,0)QtyIn,isnull(QtyOut,0)QtyOut,CarrierTb.carrierid,isnull(carriertrid,0)carriertrid,0 trOb from CarrierTb " & _
              "left join ( SELECT QtyIn,QtyOut,carrierid,carriertrid FROM CarrierTrTb where trid=" & loadedTrId & " and isnull(trid,0)>0)CarrierTrTb on CarrierTrTb.carrierid=CarrierTb.carrierid " & _
              "left join(select sum(QtyIn)obQtyIn,sum(QtyOut)obQtyOut,carrierid OBcarrierid from  CarrierTrTb where customerid=" & customerid & _
              " group by carrierid)OBCarrierTrTb on OBCarrierTrTb.OBcarrierid=CarrierTb.carrierid"
        ElseIf loadedTrId > 0 Then
            dt = _objcmnbLayer._fldDatatable("select customerid from CarrierTrTb where trid=" & loadedTrId & " group by customerid")
            If dt.Rows.Count > 0 Then
                If customerid <> Val(dt(0)("customerid") & "") And Not isfromInvoice Then GoTo n
                customerid = Val(dt(0)("customerid") & "")
            End If
            str = "select carriername,isnull(obQtyIn,0)obQtyIn,isnull(obQtyOut,0)obQtyOut,isnull(QtyIn,0)QtyIn,isnull(QtyOut,0)QtyOut,CarrierTb.carrierid,isnull(carriertrid,0)carriertrid,isnull(trOb,0)trOb from CarrierTb " & _
              "left join ( SELECT trOb,QtyIn,QtyOut,carrierid,carriertrid FROM CarrierTrTb where trid=" & loadedTrId & " and isnull(trid,0)>0)CarrierTrTb on CarrierTrTb.carrierid=CarrierTb.carrierid " & _
              "left join(select sum(QtyIn)obQtyIn,sum(QtyOut)obQtyOut,carrierid OBcarrierid from  CarrierTrTb where customerid=" & customerid & _
              " AND trid<" & loadedTrId & " group by carrierid)OBCarrierTrTb on OBCarrierTrTb.OBcarrierid=CarrierTb.carrierid"
        End If
        ...
        For i = 0 To dt.Rows.Count - 1
            With grdcarrier
                ...
                Else
                    .Item(constoldbalance, i).Value = dt(i)("trOb")
                End If
                '.Item(constoldbalance, i).Value = dt(i)("obQtyOut") - dt(i)("obQtyIn")
                .Item(constCarrierCurrentVoucher, i).Value = dt(i)("QtyOut")
                If Val(.Item(constcarrierReturn, i).Value) = 0 Then
                    .Item(constcarrierReturn, i).Value = dt(i)("QtyIn")
                End If
                .Item(constcarrierid, i).Value = dt(i)("carrierid")
                .Item(constcarriertrid, i).Value = dt(i)("carriertrid")
                .Item(constcustomerid, i).Value = customerid
                .Item(constcarrierbal, i).Value = Val(.Item(constoldbalance, i).Value) + Val(.Item(constCarrierCurrentVoucher, i).Value) - Val(.Item(constcarrierReturn, i).Value)
                chgbyprg = False
            End With
        Next
        'calculateCarrier()
    ...
    Private Sub LoadReport(ByVal FileName As String, ByVal RptCaption As String, Optional ByVal ToPrint As Boolean = False, Optional ByVal pno As Integer = 0, Optional ByVal printername As String = "")
        ...
        Dim ds As DataSet
        ds = _objInv.ldInvoice("rturnInventoryDetailsForInvoicePrintFruits", pno)
        If ToPrint Then
...
createNewItem:
    ...
    Private Sub setCashCustomer(Optional ByVal accid As Integer = 0)
        ...
        If dt.Rows.Count > 0 Then
            ...
            End If
            creditCustomerACC = Val(dt(0)("customeraccount") & "")
            loadCarriers(creditCustomerACC, False)
    ...
    End Sub
    Public Sub returnFromJob(ByVal jobcode As String, ByVal jobid As Integer, ByVal plateno As String, ByVal cartype As String, ByVal customerid As Integer)
        chgbyprg = True
        txtJob.Text = jobcode
        txtJob.Tag = jobid
...
createNewItem:
    ...
    Private Sub calculateCarrier()
        ...
        With grdcarrier
            For i = 0 To .RowCount - 1
                cqty = 0
                For r = 0 To grdVoucher.RowCount - 1
                    If Trim(.Item(constCarriername, i).Value) = Trim(grdVoucher.Item(Constsman, r).Value) Then
                        cqty = cqty + Val(grdVoucher.Item(ConstWoodQty, r).Value)
                    End If
                Next
                .Item(constCarrierCurrentVoucher, i).Value = cqty
                .Item(constcarrierbal, i).Value = Val(.Item(constoldbalance, i).Value) + Val(.Item(constCarrierCurrentVoucher, i).Value) - Val(.Item(constcarrierReturn, i).Value)
            Next
        End With
    End Sub
    ...
    Private Sub grdcarrier_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdcarrier.CellClick
        With grdcarrier
            If e.ColumnIndex = constcarrierReturn Then
    ...
    Private Sub grdcarrier_CellValueChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdcarrier.CellValueChanged
        If grdcarrier.RowCount = 0 Then Exit Sub
        If grdcarrier.CurrentRow Is Nothing Then Exit Sub
        Dim i As Integer = e.RowIndex
        With grdcarrier
            .Item(constcarrierbal, i).Value = Val(.Item(constoldbalance, i).Value) + Val(.Item(constCarrierCurrentVoucher, i).Value) - Val(.Item(constcarrierReturn, i).Value)
        End With
    End Sub
    Private Sub saveCarrier(ByVal trid As Long)
        Dim i As Integer
        Dim qry As String
        Dim trRemarks As String = "TRANSACTION FROM SALES"
        With grdcarrier
            For i = 0 To .RowCount - 1
                If Val(.Item(constcarriertrid, i).Value) = 0 Then
                    qry = "Insert into CarrierTrTb(trid,customerid,carrierid,QtyOut,QtyIn,trdate,trRemarks,trOb) values(" & _
                trid & "," & Val(.Item(constcustomerid, i).Value) & "," & Val(.Item(constcarrierid, i).Value) & "," & Val(.Item(constCarrierCurrentVoucher, i).Value) & "," & Val(.Item(constcarrierReturn, i).Value) & "," & _
                "'" & Format(DateValue(cldrdate.Value), "yyyy/MM/dd") & "','" & trRemarks & "'," & Val(.Item(constoldbalance, i).Value) & ")"
                Else
                    qry = "Update CarrierTrTb set trid=" & loadedTrId & "," & _
                                                 ...
                                                 "QtyIn=" & Val(.Item(constcarrierReturn, i).Value) & "," & _
                                                 "QtyOut=" & Val(.Item(constCarrierCurrentVoucher, i).Value) & "," & _
                                                 "trdate='" & Format(DateValue(cldrdate.Value), "yyyy/MM/dd") & "'," & _
                                                 "trRemarks='" & trRemarks & "'," & _
                                                 "trOb=" & Val(.Item(constoldbalance, i).Value) & _

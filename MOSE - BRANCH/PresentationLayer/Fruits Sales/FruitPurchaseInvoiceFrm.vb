...
Public Class FruitPurchaseInvoiceFrm
...
#Region "Local Variables"
    ...
    Private creditsupplierACC As Long
#End Region
...
#End Region
    ...
    Private Sub ldPostedInv()
        ...
        If Val(ItmInvCmnTb(0)("ccustid") & "") = 0 Then
            creditsupplierACC = 0 ' Val(ItmInvCmnTb(0)("CSCode" & ""))
        Else
            creditsupplierACC = Val(ItmInvCmnTb(0)("ccustid" & ""))
            'txtCashCustomer.Enabled = False
        End If
        loadCarriers(IIf(creditsupplierACC > 0, creditsupplierACC, Val(txtSuppAlias.Tag)), True)
        chgbyprg = True
    ...
    Private Sub txtReference_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtDescr.KeyDown, txtSuppName.KeyDown, txtSuppAlias.KeyDown, txtReference.KeyDown, txtjobname.KeyDown, txtJob.KeyDown, txtcredit.KeyDown, txtCashSuppName.KeyDown
        ...
        ElseIf e.KeyCode = Keys.F2 Then
            ...
            Select Case MyCtrl.Name
                Case "txtSuppAlias", "txtSuppName", "txtCashSuppName"
        ...
        With grdVoucher
            Select Case ColIndex
                ...
                Case ConstWoodQty, ConstWoodDiscQty
                    ...
                    End If
                Case Constsman
                    '.Item(ConstQty, RowIndex).Value = .Item(ConstWoodQty, RowIndex).Value
                    calculate(, True)
                Case ConstQty
        ...
        If Not dtSetoffTable Is Nothing Then
            If enableMultipleDebitInInvoice And dtSetoffTable.Rows.Count > 0 Then
                ...
                For i = 0 To dtSetoffTable.Rows.Count - 1
                    ...
                    If balance > 0 Then
                        setAcctrDetValue(Val(dtSetoffTable(i)("linkno")), Val(dtSetoffTable(i)("accountno")), "ON/AC", Trim(dtSetoffTable(i)("EntryRef")), balance, "", "", 0, 0, "", _
                               "", Val(txtSuppAlias.Tag), "", dtSetoffTable(i)("CurrencyCode"), dtSetoffTable(i)("CurrRate"), , Val(dtSetoffTable(i)("UnqNo")))

                        setAcctrDetValue(Val(dtSetoffTable(i)("linkno")), Val(dtSetoffTable(i)("accountno")), reference, Trim(dtSetoffTable(i)("EntryRef")), CDbl(dtSetoffTable(i)("Assign")), "", "", 0, 0, "", _
...
nxt1:
    ...
    Private Sub setAcctrDetValue(ByVal lnkNo As Long, ByVal AccountNo As Long, ByVal Reference As String, ByVal EntryRef As String, ByVal DealAmt As Double, ByVal JobCode As String, ByVal JobStr As String, _
        ...
        dtrow("Reference") = IIf(Reference = "", Trim(txtReference.Text), Reference)
...
els:
    ...
    Private Sub LoadReport(ByVal FileName As String, ByVal RptCaption As String, Optional ByVal ToPrint As Boolean = False)
        ...
        _objInv.TrType = "IP"
        Dim ds As DataSet = _objInv.ldInvoice("rturnInventoryDetailsForInvoicePrintFruits")
...
nxt:
    ...
    Private Sub btnbarcode_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnbarcode.Click
        ...
        'End If
        'itemid = Val(grdVoucher.Item(ConstItemID, grdVoucher.CurrentRow.Index).Value)
        'qty = CDbl(grdVoucher.Item(ConstQty, grdVoucher.CurrentRow.Index).Value)
        ...
        With grdVoucher
            ...
            If sRs.Rows.Count > 0 Then
                For i = 0 To sRs.Rows.Count - 1
                    ...
                    grdVoucher.Item(ConstWoodQty, i).Value = sRs(i)("WoodNetQty")
                    .Item(Constsman, i).Value = Trim(sRs(i)("Smancode") & "")
                    .Item(constItmTot, i).Value = Format((.Item(ConstQty, i).Value * CDbl(.Item(ConstActualPrice, i).Value)) - CDbl(.Item(ConstDisAmt, i).Value), lnumformat)
    ...
    Private Sub txtCashSuppName_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtCashSuppName.Validated
        ...
        If vtype = "Credit" Then
            setSupplier()
            txtDescr.Focus()
            loadCarriers(Val(txtSuppAlias.Tag), False)
        Else
            Dim dt As DataTable
            ...
            If dt.Rows.Count > 0 Then
                ...
                creditsupplierACC = Val(dt(0)("customeraccount") & "")
                loadCarriers(creditsupplierACC, False)
                chgbyprg = False
            End If
        End If
        chgbyprg = True
...
createNewItem:
    ...
    Private Sub saveCarrier(ByVal trid As Long)
        Dim i As Integer
        Dim qry As String
        Dim trRemarks As String = "TRANSACTION FROM PURCHASE"
        With grdcarrier
            For i = 0 To .RowCount - 1
                ...
                trid & "," & Val(.Item(constcustomerid, i).Value) & "," & Val(.Item(constcarrierid, i).Value) & "," & Val(.Item(constcarrierReturn, i).Value) & "," & Val(.Item(constCarrierCurrentVoucher, i).Value) & "," & _
                ...
                Else
                    qry = "Update CarrierTrTb set trid=" & loadedTrId & "," & _
                                                 ...
                                                 "QtyOut=" & Val(.Item(constcarrierReturn, i).Value) & "," & _
    ...
    End Sub
    Private Sub loadCarriers(ByVal supplierid As Long, ByVal isfromInvoice As Boolean)
        Dim dt As DataTable
...
n:
        ...
        ElseIf loadedTrId > 0 Then
            If isfromInvoice Then
                dt = _objcmnbLayer._fldDatatable("select customerid from CarrierTrTb where trid=" & loadedTrId & " group by customerid")
                If dt.Rows.Count > 0 Then
                    If supplierid <> Val(dt(0)("customerid") & "") And Not isfromInvoice Then GoTo n
                    supplierid = Val(dt(0)("customerid") & "")
                End If
            End If

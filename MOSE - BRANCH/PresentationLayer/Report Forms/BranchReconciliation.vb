...
Public Class BranchReconciliation
...
#End Region
    ...
    Private Sub SetGrid_Head()
        ...
        With grdVoucher
            ...
            .Columns("BankCode").Visible = False
    ...
    Private Sub Loadgrid()
        ...
        _dtProjection = _clsStatementReport.returnReconciliationGrid(_FromDate, _ToDate, "", Val(txtbankname.Tag), _Type)
        Dim _qurey As EnumerableRowCollection(Of DataRow)
        _qurey = From data In _dtProjection.AsEnumerable() Where data("debit") > 0 Select data
        If _qurey.Count > 0 Then
            _dtProjection = _qurey.CopyToDataTable()
        End If
nxt:
        grdVoucher.DataSource = _dtProjection
        SetGrid_Head()
        resizeGridColumn(grdVoucher, 8)
    ...
    Private Sub ShowFmlist(ByVal myCtrl As Control)
        ...
        fMList_doFocus()
        fMList.Search(txtbankname.Text)
        fMList.AssignList(txtbankname, lstKey, chgbyprg)
        _srchOnce = True
        chgbyprg = False
    End Sub
    ...
    Private Sub fMList_doSelect(ByVal ItmFlds() As String) Handles fMList.doSelect
        chgbyprg = True
        txtbankname.Text = ItmFlds(0)
        txtbankname.Tag = ItmFlds(3)
        chgbyprg = False
    End Sub

    Private Sub txtbankname_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtbankname.Validated
        Dim dt As DataTable
        dt = _objcmnbLayer._fldDatatable("Select accid from accmast where accdescr='" & txtbankname.Text & "'")
        If dt.Rows.Count > 0 Then
            txtbankname.Tag = dt(0)("accid")
            Loadgrid()
        End If
    End Sub
End Class